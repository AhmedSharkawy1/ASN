...
// https://www.asntechnology.net/menu/{slug}
void main() {
  const restaurantId = 'abc-123';

  test('slug present → canonical path URL (whole menu)', () {
    const info = QrInfo(restaurantId: restaurantId, slug: 'abosalah');
    expect(info.menuUrl(), '${AppConfig.apiBaseUrl}/menu/abosalah');
  });

  test('slug present → canonical path URL with table query', () {
    const info = QrInfo(restaurantId: restaurantId, slug: 'abosalah');
    // No label known: the id still identifies the table in the alert.
    expect(info.menuUrl(tableId: 't1'),
        '${AppConfig.apiBaseUrl}/menu/abosalah?table=t1&table_id=t1');
    ...
    expect(info.menuUrl(tableId: 't1', tableLabel: 'ترابيزة 5'),
        '${AppConfig.apiBaseUrl}/menu/abosalah?table=%D8%AA%D8%B1%D8%A7%D8%A8%D9%8A%D8%B2%D8%A9+5&table_id=t1');
  });
