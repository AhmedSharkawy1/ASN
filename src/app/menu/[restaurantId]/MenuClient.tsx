...
import type { Category, Item, RestaurantConfig } from "./menuData";
...
export default function MenuClient({
  ...
  // props, so they are simply the initial state — which also means the splash
  // paints on the very first frame instead of after a client-side round-trip.
  const [showLanding, setShowLanding] = useState(
    () => !!config.vicino_landing_enabled && isVicino
  );
  const [showLeadPopup, setShowLeadPopup] = useState(false);
  const [showGlobalSplash, setShowGlobalSplash] = useState(() => !isVicino);
  const [activeCategory, setActiveCategory] = useState<string | null>(
    () => categories[0]?.id ?? null
  );
  const [searchQuery, setSearchQuery] = useState("");

  // The splash is server-rendered now, so prefers-color-scheme cannot be read
  // during render — the server would say light, the client dark, and React
  // would flag the mismatch. Read it after mount instead.
  ...
  if (showGlobalSplash && !config?.theme?.startsWith("vicino")) {
      ...
      const isDark = config?.default_theme_mode === 'dark' || prefersDarkScheme;
                  ...
                  globalDeliveryNumbers.map((pn, i) => (
                    <a key={i} href={`tel:${pn.number}`} className="flex items-center justify-between p-4 bg-zinc-50 dark:bg-zinc-800 border border-zinc-100 dark:border-zinc-700/50 rounded-2xl active:scale-[0.97] transition-transform" dir="rtl">
                      <div className="flex flex-col text-right">
                        <span className="text-[17px] font-black text-rose-600 dark:text-rose-500" dir={language === 'ar' ? 'rtl' : 'ltr'}>
