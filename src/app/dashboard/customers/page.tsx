    ...
    return (
        <div className="flex flex-col gap-6 w-full mx-auto pb-20">
            <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                    <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white flex items-center gap-3"><Users className="w-7 h-7 text-emerald-600 dark:text-emerald-400" />{isAr ? "إدارة العملاء" : "Customers CRM"}</h1>
                    <p className="text-slate-500 dark:text-zinc-400 text-sm mt-1">{isAr ? "تتبع العملاء وتصنيفهم حسب النشاط" : "Track and classify customers by activity"}</p>
                </div>
                <div className="flex gap-2 flex-wrap">
                    ...
                    <button onClick={exportCSV} className="flex items-center gap-2 px-3 py-2.5 bg-white dark:bg-card border border-slate-200 dark:border-zinc-800/50 text-slate-700 dark:text-zinc-300 font-bold text-sm rounded-xl -blue/30"><Download className="w-4 h-4" /> CSV</button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                    { label: "إجمالي العملاء", val: customers.length, color: "text-slate-700 dark:text-zinc-300" },
                    { label: "VIP", val: vipCount, color: "text-amber-600 dark:text-amber-400" },
                    { label: "نشط", val: activeCount, color: "text-emerald-600 dark:text-emerald-400" },
                    { label: "إجمالي الإنفاق", val: formatCurrency(totalSpend), color: "text-cyan-600 dark:text-cyan-400" },
                ].map((s, i) => <div key={i} className="bg-white dark:bg-card border border-slate-200 dark:border-zinc-800/50 rounded-xl p-4"><p className="text-xs text-slate-500 dark:text-zinc-500 font-bold uppercase mb-1">{s.label}</p><p className={`text-2xl font-extrabold ${s.color}`}>{s.val}</p></div>)}
            </div>

            {/* Import/Export */}
            <div className="bg-white dark:bg-card border border-slate-200 dark:border-zinc-800/50 rounded-xl p-4">
                <h3 className="text-base font-extrabold text-slate-900 dark:text-white flex items-center gap-2 mb-3"><FileSpreadsheet className="w-4 h-4 text-emerald-600 dark:text-emerald-400" /> استيراد من Excel</h3>
                <div className="flex flex-wrap gap-3 items-center">
                    <input ref={fileRef} type="file" accept=".xlsx,.xls" onChange={handleImport} className="hidden" />
                    <button onClick={() => fileRef.current?.click()} className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-emerald-600 to-teal-600 text-slate-900 dark:text-white font-bold text-base rounded-xl shadow-lg active:scale-95"><Upload className="w-4 h-4" /> استيراد Excel</button>
                    <button onClick={downloadTemplate} className="flex items-center gap-2 px-3 py-2.5 bg-slate-100 dark:bg-zinc-800 text-slate-700 dark:text-zinc-300 font-bold text-sm rounded-xl border border-slate-300 dark:border-zinc-700"><FileSpreadsheet className="w-4 h-4" /> تحميل نموذج</button>
                </div>
                {importStatus && <div className={`mt-3 flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold ${importStatus.type === "success" ? "bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-glass-border" : "bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-500/20"}`}>{importStatus.type === "success" ? <CheckCircle2 className="w-4 h-4" /> : <AlertCircle className="w-4 h-4" />} {importStatus.message}</div>}
            </div>

            {/* Add form */}
            {showAdd && (
                <div className="bg-white dark:bg-card border border-emerald-200 dark:border-glass-border rounded-xl p-4 space-y-3">
                    <div className="flex items-center justify-between"><h3 className="text-base font-bold text-slate-900 dark:text-white">إضافة عميل جديد</h3><button onClick={() => setShowAdd(false)} className="text-slate-500 dark:text-zinc-500 hover:text-slate-900 dark:hover:text-white"><X className="w-4 h-4" /></button></div>
                    <div className="grid grid-cols-2 gap-3">
                        {[{ key: "name", label: "الاسم", dir: "rtl" }, { key: "phone", label: "الهاتف", dir: "ltr" }, { key: "address", label: "العنوان (اختياري)", dir: "rtl" }, { key: "notes", label: "ملاحظات (اختياري)", dir: "rtl" }].map(f => (
                            <input key={f.key} value={newCust[f.key as keyof typeof newCust]} onChange={e => setNewCust(v => ({ ...v, [f.key]: e.target.value }))} placeholder={f.label} dir={f.dir} className="px-3 py-2 bg-slate-100 dark:bg-black/30 border border-slate-200 dark:border-zinc-800 rounded-lg text-base text-slate-900 dark:text-white outline-none -blue/40" />
                        ))}
                    </div>
                    <div className="flex gap-2 justify-end">
                        <button onClick={() => setShowAdd(false)} className="px-4 py-2 text-slate-500 dark:text-zinc-400 text-sm font-bold">إلغاء</button>
                        <button onClick={addCustomer} className="px-4 py-2 bg-emerald-50 dark:bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 rounded-lg text-sm font-bold border border-emerald-200 dark:border-glass-border">إضافة</button>
                    </div>
                </div>
            )}
            ...
            <div className="relative w-full sm:max-w-md flex-1">
                ...
                <input value={searchQ} onChange={e => setSearchQ(e.target.value)} placeholder="بحث بالاسم أو الرقم..." className="w-full pe-10 ps-4 py-2.5 bg-white dark:bg-card border border-slate-200 dark:border-zinc-800/50 rounded-xl text-base text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-zinc-500 outline-none -blue/50" />
            </div>

            {/* Table */}
            {loading ? <div className="text-center py-12 text-slate-500 dark:text-zinc-500 animate-pulse">جاري التحميل...</div> : filtered.length === 0 ? (
                <div className="text-center py-16 text-slate-400 dark:text-zinc-600"><Users className="w-16 h-16 mx-auto mb-3 opacity-20" /><p className="font-bold">لا يوجد عملاء</p></div>
            ) : (
