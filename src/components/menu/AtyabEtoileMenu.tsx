        ...
        <div className="min-h-screen bg-white dark:bg-[#121212] text-[#333] dark:text-[#F5F5F5] antialiased" style={{ fontFamily: "'Tajawal', 'Cairo', sans-serif" }}>
                ...
                <div className="max-w-4xl mx-auto px-4 py-3 md:py-4">
                    ...
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <div className="flex items-center gap-2 cursor-pointer group" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}>
                            {config.logo_url && (
                                <div className="w-14 h-14 md:w-16 md:h-16 relative flex items-center justify-center overflow-hidden rounded-full bg-[#FDFBF7] dark:bg-[#1E1E1E] shadow-sm border border-[#EAEAEA] dark:border-[#333] p-1">
            ...
            <main className="max-w-4xl mx-auto px-4 py-8 pb-48">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
                            {cat.items.map((item) => {
                                ...
                                return (
                                    <div
                                        key={item.id}
                                        className={`bg-[#FDFBF7] dark:bg-[#1E1E1E] rounded-3xl p-3 md:p-4 shadow-sm border border-[#EAEAEA] dark:border-[#333] relative group cursor-pointer hover:shadow-md transition-shadow ${hasManyPrices ? 'col-span-2 flex flex-row gap-3' : 'flex flex-col'}`}
                                    >
                                        {/* Item Image */}
                ...
                {config.instagram_url && (
                                <a href={config.instagram_url} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full border border-white flex items-center justify-center hover:bg-white hover:text-[#B89038] transition-colors">
                                    <FaInstagram className="w-5 h-5" />
                                </a>
                            )}
                            {config.tiktok_url && (
                                <a href={config.tiktok_url} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full border border-white flex items-center justify-center hover:bg-white hover:text-[#B89038] transition-colors">
            ...
            <nav className="fixed bottom-0 left-0 right-0 z-[60] px-4 pb-8 pt-2 md:hidden">
                <div className="w-[85vw] max-w-[310px] mx-auto etoile-glass border border-[#EAEAEA] dark:border-[#333] rounded-full p-1.5 flex items-center justify-around shadow-lg relative">
                    {config.whatsapp_number && (
                ...
                {showCategoriesMenu && (
                    ...
                    >
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
                            className="w-[85vw] max-w-[310px] bg-[#FDFBF7] dark:bg-[#1E1E1E] rounded-[2rem] shadow-2xl border border-[#EAEAEA] dark:border-[#333] overflow-hidden max-h-[85vh] flex flex-col mx-auto"
                            onClick={(e) => e.stopPropagation()}
                        >
                            <div className="px-6 py-4 flex items-center justify-between border-b border-[#EAEAEA] dark:border-[#333]">
                                <h3 className="text-xl font-bold uppercase">{isAr ? "أقسام المنيو" : "Menu Categories"}</h3>
                                <button onClick={() => setShowCategoriesMenu(false)} className="w-8 h-8 rounded-full bg-[#FDFBF7] dark:bg-[#1E1E1E] border border-[#EAEAEA] dark:border-[#333] flex items-center justify-center text-[#888] hover:text-[#B89038] hover:bg-[#B89038]/5 active:scale-95 transition-all">
                                    <X className="w-4 h-4" />
                                </button>
                            </div>
                            <div className="p-4 grid grid-cols-2 gap-3 max-h-[85vh] overflow-y-auto etoile-no-scrollbar">
                                {categories.map((cat) => (
                                    <button
                                        key={cat.id}
                                        onClick={() => handleNavClick(cat.id)}
                                        className="flex flex-col items-center justify-center gap-2 bg-white dark:bg-[#121212] hover:bg-[#B89038]/5 p-4 rounded-3xl border border-[#EAEAEA] dark:border-[#333] active:scale-95 transition-all group"
                                    >
                ...
                {showCart && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex bg-black/60 items-center justify-center backdrop-blur-md py-16 px-6 mb-safe" onClick={() => setShowCart(false)}>
                        <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} className="w-[85vw] max-w-[310px] mx-auto bg-white dark:bg-[#1E1E1E] rounded-3xl shadow-2xl overflow-hidden flex flex-col h-auto max-h-[85vh]" onClick={e => e.stopPropagation()}>
                            <div className="p-4 border-b border-[#EAEAEA] dark:border-[#333] flex justify-between items-center bg-gray-50 dark:bg-black/20">
                                ...
                                <h3 className="text-lg font-bold flex items-center gap-2">{isAr ? "سلة الطلبات" : "Your Cart"} <ShoppingCart className="w-5 h-5 text-[#B89038]" /></h3>
                            </div>
                            <div className="p-5 overflow-y-auto flex-1">
                                {cart.length === 0 ? (
                                    ...
                                    <div className="space-y-4">
                                        {cart.map((c) => (
                                            <div key={c.id} className="flex gap-4 p-3 rounded-3xl border border-[#EAEAEA] dark:border-[#333] bg-gray-50 dark:bg-black/10">
                ...
                {showPaymentOptions && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        ...
                        <motion.div initial={{ scale: 0.95, opacity: 0, y: 50 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 50 }}
                            className="bg-white dark:bg-[#1E1E1E] w-[85vw] max-w-[310px] rounded-[2rem] shadow-2xl flex flex-col overflow-hidden max-h-[85vh] border border-[#EAEAEA] dark:border-[#333] mx-auto"
                            onClick={(e) => e.stopPropagation()}>
