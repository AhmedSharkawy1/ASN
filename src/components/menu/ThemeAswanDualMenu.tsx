...
import { ShoppingCart, Plus, Minus, Trash2, X, Search, Share2, Home, Tag, Moon, Sun, LayoutGrid, LayoutList, CreditCard, ArrowLeft, ArrowRight, Check, Phone, MapPin, Clock, Globe } from 'lucide-react';
...
interface ThemeAswanDualMenuProps {
    config: RestaurantType;
    categories: CategoryWithItemsType[];
    restaurantId: string;
}

export default function ThemeAswanDualMenu({ config, categories, restaurantId }: ThemeAswanDualMenuProps) {
    ...
    // Language getters
    const getItemName = (item: MenuItem) => isAr ? (item.title_ar || item.title_en || 'صنف') : (item.title_en || item.title_ar || 'Item');
    const getItemDesc = (item: MenuItem) => isAr ? (item.description_ar || item.desc_ar || item.details_ar || item.description || item.desc || item.description_en || item.desc_en || '') : (item.description_en || item.desc_en || item.details_en || item.description || item.desc || item.description_ar || item.desc_ar || '');
    const getCatName = (cat: CategoryWithItemsType) => isAr ? (cat.name_ar || cat.name_en || 'قسم') : (cat.name_en || cat.name_ar || 'Category');
    ...
    const handleShare = async () => {
        ...
        if (navigator.share) {
            ...
            alert(isAr ? 'تم نسخ رابط القائمة بالحافظة!' : 'Menu link copied to clipboard!');
        }
    };
    ...
    if (!inMenu && landingEnabled) {
        return <AswanDualLandingPage config={config} onContinue={() => setInMenu(true)} />;
    }
            ...
            <div className="relative z-10">
                ...
                {config.marquee_enabled && (
                    <div className="text-xs md:text-sm text-white py-1.5 font-semibold tracking-normal" style={{ backgroundColor: primaryColor }}>
                        <SharedMarquee text={isAr ? (config.marquee_text_ar || config.marquee_text_en || 'أهلاً بكم في مطعمنا! استمتعوا بأشهى المأكولات والمشروبات.') : (config.marquee_text_en || config.marquee_text_ar || 'Welcome to our restaurant! Enjoy our delightful menu.')} direction={isAr ? "rtl" : "ltr"} />
                    </div>
                )}
                ...
                <div className="px-5 pt-6 pb-4 max-w-4xl mx-auto">
                    ...
                    <div className="flex justify-between items-center gap-1.5 sm:gap-2 mb-6 max-w-full overflow-x-auto no-scrollbar py-0.5">
                        ...
                        <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
                            ...
                            <button
                                ...
                                className="px-2.5 py-1.5 sm:px-3 sm:py-2 rounded-xl sm:rounded-2xl border font-bold text-xs flex items-center gap-1 shadow-sm transition-transform active:scale-95 flex-shrink-0"
                                style={{ backgroundColor: bgCard, borderColor: borderColor, color: primaryColor }}
                            >
                            ...
                            {config.payment_methods && config.payment_methods.length > 0 && (
                                <button 
                                    ...
                                    style={{ backgroundColor: bgCard, borderColor: borderColor }}
                                    title={isAr ? "خيارات الدفع" : "Payments"}
                                >
                    ...
                    <div className="relative max-w-4xl mx-auto mt-2">
                        <div className={`absolute inset-y-0 ${isAr ? 'right-4' : 'left-4'} flex items-center pointer-events-none`}>
                        ...
                        <input 
                            ...
                            className={`w-full h-12 rounded-2xl ${isAr ? 'pr-12 pl-4' : 'pl-12 pr-4'} outline-none font-medium text-sm shadow-md border transition-all focus:ring-2`}
                            style={{ backgroundColor: bgCard, color: textMain, borderColor: borderColor }}
                        />
                        {searchQuery && (
                            <button onClick={() => setSearchQuery('')} className={`absolute inset-y-0 ${isAr ? 'left-4' : 'right-4'} flex items-center`}>
                    ...
                    <div className="px-5 mb-6 max-w-4xl mx-auto">
                        <Swiper modules={[Autoplay]} autoplay={{ delay: 3500, disableOnInteraction: false }} className="w-full rounded-3xl overflow-hidden shadow-lg border" style={{ borderColor: borderColor }}>
                            {config.cover_images.map((img, i) => (
                                <SwiperSlide key={i}>
                                    <OptimizedMenuImage src={img} alt="Banner" className="w-full h-[160px] md:h-[220px] object-cover" />
                                </SwiperSlide>
                            ))}
                        </Swiper>
                    </div>
                    ...
                    <div className="mb-8 max-w-4xl mx-auto">
                        <div className="px-5 flex items-center justify-between mb-3">
                            <div className="flex items-center gap-2">
                                ...
                                <h2 className="text-lg font-bold">{isAr ? 'العروض المميزة' : 'Featured Deals'}</h2>
                            </div>
                        </div>
                                                    ...
                                                    <div className="mt-3 flex items-center justify-between">
                                                        <div className="font-extrabold text-sm md:text-base whitespace-nowrap" style={{ color: primaryColor }}>
                                                            {item.size_labels && item.size_labels.length > 1 ? (
                                                                <span className="text-xs">{isAr ? 'من ' : 'From '}{item.prices?.[0] || 0} {cur}</span>
                                                            ) : (
                        ...
                        >
                            ...
                            >
                                <div className="relative h-48 md:h-56 w-full bg-slate-900 shrink-0">
                                    ...
                                    <button 
                                        onClick={closeModal}
                                        className={`absolute top-3 ${isAr ? 'left-3' : 'right-3'} w-9 h-9 rounded-full bg-black/60 text-white flex items-center justify-center backdrop-blur-md transition-transform active:scale-95 shadow-md`}
                                    >
                                        <X className="w-5 h-5" />
                                    </button>
                                </div>
                                ...
                                <div className="p-5 overflow-y-auto space-y-5 flex-1">
                                    ...
                                    {selectedItem.item.prices && selectedItem.item.prices.length > 1 && (
                                        <div className="space-y-2.5">
                                            <label className="font-bold text-xs uppercase tracking-wider block opacity-70">{isAr ? 'اختر الحجم' : 'Select Size'}</label>
                                            <div className="grid grid-cols-2 gap-2">
                                                {selectedItem.item.prices.map((p, idx) => (
                                                    ...
                                                    >
                                                        <span>{selectedItem.item.size_labels?.[idx] || (isAr ? `خيار ${idx + 1}` : `Size ${idx + 1}`)}</span>
                                    ...
                                    {selectedItem.item.extras && selectedItem.item.extras.length > 0 && (
                                        <div className="space-y-2.5">
                                            <label className="font-bold text-xs uppercase tracking-wider block opacity-70">{isAr ? 'الإضافات والخيارات' : 'Add-ons & Extras'}</label>
                                            <div className="space-y-2">
                                                {selectedItem.item.extras.map((ext, idx) => {
                                                    const extName = isAr ? (ext.name_ar || ext.name_en || 'إضافة') : (ext.name_en || ext.name_ar || 'Extra');
                                    ...
                                    <div className="space-y-1.5">
                                        <label className="font-bold text-xs uppercase tracking-wider block opacity-70">{isAr ? 'ملاحظات خاصة' : 'Special Instructions'}</label>
                                        <textarea
                                            value={notes}
                                ...
                                <div className="p-4 border-t flex items-center gap-3 shrink-0" style={{ borderColor: borderColor }}>
                                    ...
                                    <button
                                        ...
                                        style={{ backgroundColor: primaryColor }}
                                    >
                                        <span className="whitespace-nowrap">{isAr ? 'إضافة إلى السلة' : 'Add to Cart'}</span>
                    ...
                    <div className="fixed bottom-6 inset-x-5 max-w-md mx-auto z-40">
                        ...
                        >
                            <div className="flex items-center gap-3">
                                ...
                                <span className="font-extrabold text-lg">{isAr ? 'عرض السلة' : 'View Cart'}</span>
                            </div>
                            ...
                            >
                                <div className="flex justify-between items-center mb-3">
                                    <h3 className="font-bold text-base flex items-center gap-2">
                                        ...
                                        <span>{isAr ? 'خيارات الدفع والتحويل' : 'Payment Options'}</span>
                                    </h3>
                                ...
                                <p className="text-xs opacity-70 mb-3 font-normal">
                                    {isAr ? 'يرجى إرسال صورة إيصال التحويل على الواتساب بعد إتمام عملية الدفع.' : 'Please send receipt screenshot via WhatsApp after payment.'}
                                </p>

                                {config.whatsapp_number && (
                                    <a
                                        href={`https://wa.me/${config.whatsapp_number.replace(/\+/g, '')}?text=${encodeURIComponent(isAr ? 'مرحباً، لقد قمت بإتمام عملية التحويل المالي. إليكم صورة الإيصال:' : 'Hello, I completed the payment transfer. Here is my receipt:')}`}
                                        target="_blank"
                                        rel="noopener noreferrer"
                                    ...
                                    >
                                        <FaWhatsapp className="w-5 h-5" />
                                        <span>{isAr ? 'إرسال إيصال التحويل عبر الواتساب' : 'Send Receipt via WhatsApp'}</span>
                                    </a>
                                )}
                                ...
                                <div className="space-y-3">
                                    {config.payment_methods?.map((pm: any, idx: number) => (
                                        <div key={idx} className="p-3.5 rounded-2xl border space-y-2" style={{ borderColor: borderColor, backgroundColor: bgBody }}>
                                            <h4 className="font-bold text-sm">{isAr ? (pm.name_ar || pm.name_en) : (pm.name_en || pm.name_ar)}</h4>
                                            {(pm.desc_ar || pm.desc_en) && <p className="text-xs opacity-70 font-normal">{isAr ? (pm.desc_ar || pm.desc_en) : (pm.desc_en || pm.desc_ar)}</p>}
                                            {pm.number && (
