 ...
 */
export default function AtyabEtoileRedMenu({ config, categories, language, restaurantId }: Props) {
    ...
    return (
        <div className="min-h-screen bg-white dark:bg-[#121212] text-[#333] dark:text-[#F5F5F5] antialiased" style={{ fontFamily: "'Tajawal', 'Cairo', sans-serif" }}>
            ...
            <header className="relative z-50 bg-[#FDFBF7] dark:bg-[#1E1E1E] shadow-sm pt-[env(safe-area-inset-top,0px)]">
                <div className="max-w-4xl mx-auto px-4 py-3 md:py-4">
                    ...
                    <div className="flex items-center justify-between mb-4">
                        {/* Hamburger */}
                        <button onClick={() => { haptic(); setShowCategoriesMenu(true); }} className="w-10 h-10 border border-[#22d3ee] dark:border-[#333] rounded-md flex items-center justify-center text-[#888] hover:text-[#dc2626] transition-colors active:scale-95">
                        ...
                        <div className="flex items-center gap-2 cursor-pointer group" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}>
                            {config.logo_url && (
                                <div className="w-14 h-14 md:w-16 md:h-16 relative flex items-center justify-center overflow-hidden rounded-full bg-[#FDFBF7] dark:bg-[#1E1E1E] shadow-sm border border-[#22d3ee] dark:border-[#333] p-1">
                        ...
                        {/* Dark mode toggle */}
                        <button onClick={toggleDarkMode} className="w-10 h-10 rounded-full flex items-center justify-center text-lg text-[#888] hover:text-[#dc2626] hover:bg-[#dc2626]/5 transition-colors active:scale-95 border border-[#22d3ee] dark:border-[#333]" suppressHydrationWarning>
                            {isDark ? "☀️" : "🌙"}
                        </button>
                    </div>
                    ...
                    <div className="relative">
                        <input
                            ...
                            className="w-full bg-transparent border border-[#22d3ee] dark:border-[#333] rounded-full py-2.5 px-6 text-sm outline-none focus:border-[#dc2626] transition-colors text-right"
                            dir="rtl"
                        />
            ...
            <nav className="sticky top-0 z-40 bg-white/95 dark:bg-[#121212]/95 backdrop-blur-md border-b border-[#22d3ee] dark:border-[#333] shadow-sm mt-6">
                <div className="max-w-4xl mx-auto relative flex items-center">
                    <button onClick={() => scrollNav("right")} className="absolute right-0 z-10 w-12 h-full bg-gradient-to-l from-white dark:from-[#121212] to-transparent flex items-center justify-center text-[#888] hover:text-[#dc2626] transition-colors">
                        ...
                        {categories.map((cat) => (
                            <button
                                ...
                                className={`whitespace-nowrap px-4 py-2 rounded-full text-sm font-bold transition-colors ${activeSection === cat.id
                                    ...
                                    : "bg-[#FDFBF7] dark:bg-[#1E1E1E] text-[#888] hover:text-[#dc2626] border border-[#22d3ee] dark:border-[#333]"
                                    }`}
                    ...
                    <button onClick={() => scrollNav("left")} className="absolute left-0 z-10 w-12 h-full bg-gradient-to-r from-white dark:from-[#121212] to-transparent flex items-center justify-center text-[#888] hover:text-[#dc2626] transition-colors">
                                    ...
                                    >
                                        ...
                                        <div className={`overflow-hidden relative shrink-0 ${hasManyPrices ? 'w-28 md:w-36 rounded-xl' : 'w-full h-32 md:h-40'}`} style={{ backgroundColor: '#FDFBF7' }}>
                                            ...
                                            <button
                                                ...
                                                className={`absolute top-2 right-2 w-8 h-8 bg-white/90 rounded-full flex items-center justify-center shadow-sm transition-colors active:scale-95 ${favorites.includes(item.id) ? "text-red-500" : "text-[#888] hover:text-[#dc2626]"}`}
                                            >
                                                ...
                                                {item.prices.length > 1 ? (
                                                    <div className="w-full space-y-1 mb-3">
                                                        {item.prices.map((price, idx) => {
                                                            ...
                                                            return (
                                                                <div key={idx} className="flex justify-between items-center w-full text-xs bg-gray-50 dark:bg-black/20 px-2 py-1 rounded-md border border-[#22d3ee] dark:border-[#333]">
                ...
                <footer className="mt-16 rounded-[2rem] text-white p-6 sm:p-8" style={{ backgroundColor: PRIMARY }}>
                    <div className="max-w-4xl mx-auto flex flex-col items-center text-center gap-8">
                        ...
                        <div className="flex gap-6">
                            {config.map_link && (
                                <a href={config.map_link} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full border border-white flex items-center justify-center hover:bg-white hover:text-[#dc2626] transition-colors">
                            ...
                            {config.facebook_url && (
                                <a href={config.facebook_url} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full border border-white flex items-center justify-center hover:bg-white hover:text-[#dc2626] transition-colors">
                                    <FaFacebook className="w-5 h-5" />
                                </a>
                            )}
                {config.instagram_url && (
                                <a href={config.instagram_url} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full border border-white flex items-center justify-center hover:bg-white hover:text-[#dc2626] transition-colors">
                                    <FaInstagram className="w-5 h-5" />
                                </a>
                            )}
                            {config.tiktok_url && (
                                <a href={config.tiktok_url} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full border border-white flex items-center justify-center hover:bg-white hover:text-[#dc2626] transition-colors">
            ...
            <nav className="fixed bottom-0 left-0 right-0 z-[60] px-4 pb-8 pt-2 md:hidden">
                <div className="w-[85vw] max-w-[310px] mx-auto etoile-glass border border-[#22d3ee] dark:border-[#333] rounded-full p-1.5 flex items-center justify-around shadow-lg relative">
                    ...
                    <button onClick={() => { haptic(); setShowCallMenu(!showCallMenu); setShowCategoriesMenu(false); }} className={`flex-1 flex flex-col items-center py-2 active:scale-95 transition-all ${showCallMenu ? "text-[#dc2626]" : "text-[#888]"}`}>
                    ...
                    <button onClick={() => { setShowCategoriesMenu(true); setShowCallMenu(false); }} className={`flex-1 flex flex-col items-center py-2 active:scale-95 transition-all ${showCategoriesMenu ? "text-[#dc2626]" : "text-[#888]"}`}>
                        ...
                        {showCallMenu && (
                            ...
                            >
                                ...
                                {config.phone_numbers?.map((p, i) => (
                                    <a key={i} href={`tel:${p.number}`} className="flex items-center justify-between px-6 py-4 border-b last:border-0 border-[#22d3ee] dark:border-[#333] active:bg-[#dc2626]/5 transition-colors group">
            ...
            <AnimatePresence>
                {showCategoriesMenu && (
                    ...
                    >
                        ...
                        >
                            <div className="px-6 py-4 flex items-center justify-between border-b border-[#22d3ee] dark:border-[#333]">
                                <h3 className="text-xl font-bold uppercase">{isAr ? "أقسام المنيو" : "Menu Categories"}</h3>
                                <button onClick={() => setShowCategoriesMenu(false)} className="w-8 h-8 rounded-full bg-[#FDFBF7] dark:bg-[#1E1E1E] border border-[#22d3ee] dark:border-[#333] flex items-center justify-center text-[#888] hover:text-[#dc2626] hover:bg-[#dc2626]/5 active:scale-95 transition-all">
                                    <X className="w-4 h-4" />
                                </button>
                            </div>
                            <div className="p-4 grid grid-cols-2 gap-3 max-h-[85vh] overflow-y-auto etoile-no-scrollbar">
                                {categories.map((cat) => (
                                    <button
                                        key={cat.id}
                                        onClick={() => handleNavClick(cat.id)}
                                        className="flex flex-col items-center justify-center gap-2 bg-white dark:bg-[#121212] hover:bg-[#dc2626]/5 p-4 rounded-3xl border border-[#22d3ee] dark:border-[#333] active:scale-95 transition-all group"
                                    >
                                        <span className="text-3xl">{cat.emoji || "✨"}</span>
                                        <span className="text-[13px] font-semibold group-hover:text-[#dc2626] uppercase text-center leading-tight">
                                            {isAr ? cat.name_ar : cat.name_en || cat.name_ar}
                                        </span>
                                    </button>
                                ))}
                ...
                {selectedItem && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex bg-black/60 items-center justify-center backdrop-blur-md py-16 px-6 mb-safe" onClick={() => { setSelectedItem(null); setItemNotes(''); }}>
                        <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} className="w-[85vw] max-w-[310px] mx-auto bg-white dark:bg-[#1E1E1E] rounded-3xl shadow-2xl overflow-hidden  max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
                            ...
                            <div className="p-5 flex-1 overflow-y-auto">
                                ...
                                <div className="space-y-2">
                                    {selectedItem.item.prices.map((p, idx) => {
                                        ...
                                        return (
                                            <div key={idx} onClick={() => { haptic(5); setTempSizeIdx(idx); }} className={`flex justify-between items-center p-4 rounded-xl border-2 cursor-pointer transition-all ${isSelected ? "border-[#dc2626] bg-[#dc2626]/5" : "border-[#22d3ee] dark:border-[#333] hover:border-[#dc2626]/50"}`}>
                                                <div className="flex items-center gap-3">
                                                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${isSelected ? "border-[#dc2626]" : "border-gray-300"}`}>
                            ...
                            <div className="p-4 bg-gray-50 dark:bg-black/20 border-t border-[#22d3ee] dark:border-[#333]">
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex bg-black/60 items-center justify-center backdrop-blur-md py-16 px-6 mb-safe" onClick={() => setShowCart(false)}>
                        <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} className="w-[85vw] max-w-[310px] mx-auto bg-white dark:bg-[#1E1E1E] rounded-3xl shadow-2xl overflow-hidden flex flex-col h-auto max-h-[85vh]" onClick={e => e.stopPropagation()}>
                            <div className="p-4 border-b border-[#22d3ee] dark:border-[#333] flex justify-between items-center bg-gray-50 dark:bg-black/20">
                                ...
                                <h3 className="text-lg font-bold flex items-center gap-2">{isAr ? "سلة الطلبات" : "Your Cart"} <ShoppingCart className="w-5 h-5 text-[#dc2626]" /></h3>
                            </div>
                                        ...
                                        {cart.map((c) => (
                                            <div key={c.id} className="flex gap-4 p-3 rounded-3xl border border-[#22d3ee] dark:border-[#333] bg-gray-50 dark:bg-black/10">
                                                <div className="flex flex-col items-center justify-between bg-white dark:bg-[#1E1E1E] rounded-full p-1 border border-[#22d3ee] dark:border-[#333]">
                                                    <button onClick={() => updateCartQty(c.id, 1)} className="w-7 h-7 rounded-full bg-gray-100 dark:bg-[#333] flex items-center justify-center hover:bg-[#dc2626] hover:text-white transition-colors" >+</button>
                                                ...
                                                <div className="flex-1 text-right flex flex-col justify-center">
                                                    <div className="flex justify-between items-start mb-1 flex-row-reverse">
                                                        ...
                                                        <span className="font-bold text-[#dc2626]">{c.price * c.quantity} {currency}</span>
                                                    </div>
                            ...
                            {cart.length > 0 && (
                                <div className="p-5 border-t border-[#22d3ee] dark:border-[#333] bg-gray-50 dark:bg-black/20">
                                    <div className="flex justify-between items-center mb-4">
                                        <span className="font-bold text-[#dc2626] text-xl">{cartTotal} {currency}</span>
                        ...
                        <motion.div initial={{ scale: 0.95, opacity: 0, y: 50 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 50 }}
                            ...
                            <div className="p-4 overflow-y-auto flex flex-col gap-3">
                                {config.payment_methods?.map((pm) => (
                                    <div key={pm.id} className="bg-[#FDFBF7] dark:bg-[#121212] border border-[#22d3ee] dark:border-[#333] rounded-[1.5rem] p-4 text-right flex flex-col transition-all hover:border-[#dc2626]/50">
                                        ...
                                        {pm.number && (
                                            <div className="flex items-center justify-between bg-white dark:bg-[#1E1E1E] px-4 py-3 rounded-xl mb-3 border border-[#22d3ee] dark:border-[#333] group">
                                                <button
                                                    onClick={() => {
                                                        navigator.clipboard.writeText(pm.number!);
                                                        alert(isAr ? "تم نسخ الرقم!" : "Number copied!");
                                                    }}
                                                    className="text-[#dc2626] text-xs font-bold uppercase bg-[#dc2626]/10 px-3 py-1.5 rounded-lg active:scale-95"
                                                >
                                                    {isAr ? "نسخ" : "Copy"}
                                                </button>
