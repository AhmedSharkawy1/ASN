...
const THEME5_PRIMARY = '#059669'; // Light blue
const THEME5_LIGHT = '#34d399';
const THEME5_GREEN = '#a7f3d0';
const THEME5_GREEN_TEXT = '#059669';

export default function Theme5EmeraldMenu({ config, categories, language, restaurantId }: Props) {
                ...
                <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-100 dark:border-zinc-800 h-14">
                    <div className="flex items-center gap-4">
                        {isSearchOpen ? (
                            ...
                            <button onClick={() => setIsSearchOpen(true)} className="text-zinc-600 dark:text-zinc-300 transition-colors hover:text-[#059669]">
                                <Search className="w-5 h-5" />
                            </button>
                        )}
                        {!isSearchOpen && (
                            <button onClick={handleShare} className="text-zinc-600 dark:text-zinc-300 transition-colors hover:text-[#059669]">
                            ...
                            <div className="p-6 overflow-y-auto">
                                <div className="space-y-3">
                                    {selectedItem.item.prices.map((price, idx) => {
                                        ...
                                        return (
                                            <div key={idx} onClick={() => setTempSizeIdx(idx)} className={`flex justify-between items-center p-4 rounded-xl border-2 cursor-pointer flex-row-reverse ${isSelected ? 'border-[#059669] bg-[#34d399] dark:bg-[#059669]/10' : 'border-zinc-200 dark:border-zinc-800'}`}>
                                                <div className="flex items-center gap-3 flex-row-reverse">
                                                    ...
                                                    <span className={`font-bold ${isSelected ? 'text-[#059669]' : ''}`}>{label}</span>
                                                </div>
                                                <span className={`font-bold ${isSelected ? 'text-[#059669]' : ''}`}>{price} {currency}</span>
                                            </div>
                                        );
                                    })}
                                </div>
                                ...
                                <div className="mt-4">
                                    <textarea
                                        ...
                                        className="w-full p-3 rounded-xl border-2 border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 text-sm text-zinc-800 dark:text-zinc-200 placeholder-zinc-400 dark:placeholder-zinc-500 outline-none focus:border-[#059669] transition-colors resize-none"
                                        rows={2}
                                        dir={isAr ? "rtl" : "ltr"}
                                    />
                                </div>
