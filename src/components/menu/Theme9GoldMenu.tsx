...
// ================= THEME 9 GOLD CONSTANTS =================
// The main accent color - Gold
const T9_RED = '#D4A017';
...
export default function Theme9GoldMenu({ config, categories, restaurantId }: Theme9MenuProps) {
                                                ...
                                                <div className=" flex flex-col gap-2 w-full" dir={isAr ? 'rtl' : 'ltr'}>
                                                    <div className="flex flex-col gap-1 w-full text-right mb-2 px-1">
                                                        {item.prices.map((p: number, idx: number) => {
                                                            const sizeLabel = item.size_labels && item.size_labels[idx] ? item.size_labels[idx] : null;
                                                            return (
                                                                <div key={idx} className={`flex ${sizeLabel ? 'justify-between' : 'justify-end'} items-center w-full`}>
                                                                    {sizeLabel && (
                                            ...
                                            {selectedItem.item.prices.map((_p: number, idx: number) => {
                                                ...
                                                return (
                                                    <label key={idx} className="flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-colors"
                                                        style={{ borderColor: sizeIdx === idx ? T9_RED : borderColor, background: sizeIdx === idx ? (isDark ? 'rgba(231,76,60,0.1)' : '#22d3ee') : 'transparent' }}
                                    ...
                                    <div className="mb-6">
                                        ...
                                        <div className="space-y-2">
                                            {selectedItem.item.extras.map((ext: { id?: number | string, name_ar: string, name_en?: string, price: number }, idx: number) => {
                                                ...
                                                return (
                                                    <label key={idx} className="flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-colors"
                                                        style={{ borderColor: isSel ? T9_RED : borderColor, background: isSel ? (isDark ? 'rgba(231,76,60,0.1)' : '#22d3ee') : 'transparent' }}>
                        ...
                        <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }}
                            ...
                            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-1">
                                {[
                                    ...
                                    { label: isAr ? 'أقسام وأصناف' : 'Menu', icon: <UtensilsCrossed className="w-5 h-5" />, action: () => { document.querySelector('nav')?.scrollIntoView(); setIsDrawerOpen(false); } },
                    ...
                    {(config.phone_numbers && config.phone_numbers.length > 0) && (
                        <div className="relative shrink-0">
                            <button
                                ...
                                className="w-10 h-10 rounded-full flex items-center justify-center bg-gray-100 dark:bg-slate-800 text-[#0891b2] hover:bg-red-50 dark:hover:bg-slate-700 transition-colors border border-gray-200 dark:border-slate-700 shadow-sm"
                            >
