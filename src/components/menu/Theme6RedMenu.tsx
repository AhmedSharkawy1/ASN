...
/* ───────────────────── THEME 6 CONSTANTS ───────────────────── */
const T6 = '#dc2626';        // brand-primary teal

/* ───────────────────── COMPONENT ───────────────────── */
export default function Theme6RedMenu({ config, categories, restaurantId }: { config: any; categories: any[]; restaurantId: string }) {
                ...
                <div ref={catNavRef}
                    ...
                    <button onClick={scrollToTop} data-cat-id="top"
                        ...
                        <div className={`w-16 h-16 rounded-full flex items-center justify-center text-xl shadow-sm border-2 transition-colors
                            ${activeSection === '' ? 'border-[#dc2626] bg-[#22d3ee]' : 'border-transparent bg-zinc-100 dark:bg-zinc-800'}`}>
                            🏠
                        </div>
                        <span className={`text-sm font-bold transition-colors ${activeSection === '' ? 'text-[#dc2626]' : 'text-zinc-600 dark:text-zinc-400'}`}>
                            {isAr ? 'الكل' : 'All'}
                        </span>
                    </button>
                    ...
                    {categories.map((s: any) => (
                        <button key={s.id} data-cat-id={s.id}
                            ...
                            <div className={`w-16 h-16 rounded-full overflow-hidden shadow-sm border-2 transition-colors
                                ${activeSection === s.id ? 'border-[#dc2626]' : 'border-transparent'}`}>
                                {s.image_url
                            ...
                            <span className={`text-sm font-bold transition-colors ${activeSection === s.id ? 'text-[#dc2626]' : 'text-zinc-600 dark:text-zinc-400'}`}>
                                                ...
                                                return (
                                                    <label key={idx}
                                                        className={`flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-colors
                                                               ${sel ? 'border-[#dc2626] bg-[#22d3ee]/50' : 'border-zinc-200 dark:border-zinc-700'}`}>
                                                        <div className="flex items-center gap-1">
