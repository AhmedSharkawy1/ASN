import { getUaeColors } from '@/lib/uaeVariants';
import React from 'react';
import { useTheme } from 'next-themes';
import { ArrowLeft, Phone, Clock, Instagram, Facebook, Youtube, Sun, Moon, Sparkles, ChefHat, Award } from 'lucide-react';
...
interface UaeLandingPageProps {
    config: any;
    onContinue: () => void;
}

export default function UaeLandingPage({ config, onContinue }: UaeLandingPageProps) {
    ...
    const { primaryColor, bgBody, textMain, textMuted, bgCard, borderColor, hasBgImage, activeBgImage } = getUaeColors(config, isDark);
    ...
    const socialLinks = [
        ...
        { icon: FaTiktok, url: config.tiktok_url, color: isDark ? '#ffffff' : '#000000', name: 'تيك توك', textColor: isDark ? '#ffffff' : '#000000' },
        { icon: Facebook, url: config.facebook_url, color: '#1877F2', name: 'فيسبوك' },
        { icon: FaSnapchatGhost, url: config.snapchat_url, color: '#FFFC00', name: 'سناب شات', textColor: isDark ? '#ffffff' : '#000000' },
        ...
        return (
            <div className="fixed inset-0 z-[9999] bg-slate-950 flex items-center justify-center">
                <div className="relative w-48 h-48 flex items-center justify-center">
                    <div className="absolute inset-0 rounded-full border-[4px] border-amber-500/20 border-t-amber-500 animate-spin"></div>
    ...
    return (
        ...
        >
            ...
            {/* Header & Prominent Centered Brand Logo (UAE Theme) */}
            <div className="w-full max-w-4xl mx-auto px-6 pt-6 pb-4 text-center flex flex-col items-center justify-center">
                ...
                {finalLogoSrc && (
                    <div className="relative mb-4 group">
                        ...
                        <div className={`relative w-24 h-24 sm:w-28 sm:h-28 md:w-32 md:h-32 rounded-full overflow-hidden border-4 border-amber-500 shadow-2xl flex items-center justify-center p-2 backdrop-blur-md ${
                ...
                <span className="text-xs sm:text-sm font-extrabold text-amber-500 tracking-widest uppercase">ثيم الإمارات 🇦🇪 - تجربة فاخرة</span>
            </div>
            ...
            <div className="w-full px-6 max-w-4xl mx-auto mb-10 text-center">
                ...
                >
                    <span>تصفح المنيو واطلب أونلاين</span>
                ...
                <div className="w-full px-6 max-w-4xl mx-auto mb-12 space-y-8">
                    ...
                    {(config.vicino_about_ar || config.vicino_about_en) && (
                        ...
                        >
                            <div className="flex items-center gap-2 text-amber-500 font-bold text-xs uppercase tracking-widest">
                                ...
                                <span>عن المكان وجودة التقديم</span>
                            </div>
                            <h2 className="text-xl md:text-2xl font-extrabold tracking-tight">عن {config.name}</h2>
                    ...
                    {(config.vicino_history_ar || config.vicino_history_en) && (
                        ...
                        >
                            <div className="flex items-center gap-2 text-amber-500 font-bold text-xs uppercase tracking-widest">
                                ...
                                <span>القصة والعراقة</span>
                            </div>
                            <h2 className="text-xl md:text-2xl font-extrabold tracking-tight">قصتنا</h2>
                            <p className="text-sm md:text-base leading-relaxed opacity-90 font-normal">
                                {config.vicino_history_ar || config.vicino_history_en}
                            </p>
                        </motion.div>
                    ...
                    {config.vicino_images && config.vicino_images.length > 0 && (
                        <motion.div 
                            ...
                            className="space-y-4"
                        >
                            <div className="flex items-center gap-2 text-amber-500 font-bold text-xs uppercase tracking-widest px-1">
            ...
            {/* Info Cards: Work Hours & Direct Contact */}
            <div className="w-full px-6 max-w-4xl mx-auto mb-12 grid grid-cols-1 md:grid-cols-2 gap-4">
                
                {(config.work_hours_ar || config.work_hours_en || config.opening_hours) && (
                    <div 
                    ...
                    >
                        ...
                        <div className="space-y-1">
                            <h3 className="font-bold text-base">ساعات العمل</h3>
                ...
                {displayNumbers.length > 0 && (
                    <div 
                        onClick={() => setShowPhoneModal(true)}
                        className={`p-6 rounded-3xl border shadow-md flex items-start gap-4 cursor-pointer hover:border-amber-500/50 transition-all ${
                            hasBgImage 
                                ? (isDark ? 'bg-slate-900/90 backdrop-blur-md' : 'bg-white/95 backdrop-blur-md')
                                : ''
                        }`}
                    ...
                    >
                        ...
                        <div className="space-y-1">
                            <h3 className="font-bold text-base">التواصل والخط الساخن</h3>
                            <p className="text-xs opacity-75">اضغط للاتصال أو استعراض أرقام الهاتف المتاحة</p>
                        </div>
                    </div>
                )}
            ...
            {socialLinks.length > 0 && (
                <div className="w-full px-6 max-w-4xl mx-auto mb-12 text-center space-y-4">
                    <span className="text-xs font-semibold uppercase tracking-wider opacity-75">تابعنا على وسائل التواصل</span>
            ...
            <div className="w-full py-8 text-center border-t text-xs opacity-60" style={{ borderColor }}>
                <p>&copy; {new Date().getFullYear()} {config.name}. جميع الحقوق محفوظة.</p>
            </div>
                ...
                {showPhoneModal && (
                    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
                        <motion.div 
                            ...
                            className={`w-full max-w-sm rounded-3xl p-6 border shadow-2xl space-y-4 text-right ${
                                isDark ? 'bg-slate-900 border-slate-800 text-slate-100' : 'bg-white border-slate-200 text-slate-900'
                            }`}
                            style={{ borderColor }}
                        >
                            <div className="flex justify-between items-center border-b pb-3" style={{ borderColor }}>
                                <h3 className="font-bold text-base">اتصل بنا مباشرة</h3>
                            ...
                            <div className="space-y-2">
                                {displayNumbers.map((numObj: any, idx: number) => (
                                    ...
                                    >
                                        <span>{numObj.label || 'الهاتف'}</span>
                                        <span className="text-amber-500 font-bold ltr">{numObj.number}</span>
                                    </a>
                                ))}
                            </div>
                        </motion.div>
