...
export default function Theme9EmeraldMenu({ config, categories, restaurantId }: Theme9MenuProps) {
                                            ...
                                            {selectedItem.item.prices.map((_p: number, idx: number) => {
                                                ...
                                                return (
                                                    <label key={idx} className="flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-colors"
                                                        style={{ borderColor: sizeIdx === idx ? T9_RED : borderColor, background: sizeIdx === idx ? (isDark ? 'rgba(231,76,60,0.1)' : '#34d399') : 'transparent' }}
                                    ...
                                    <div className="mb-6">
                                        ...
                                        <div className="space-y-2">
                                            {selectedItem.item.extras.map((ext: { id?: number | string, name_ar: string, name_en?: string, price: number }, idx: number) => {
                                                ...
                                                return (
                                                    <label key={idx} className="flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-colors"
                                                        style={{ borderColor: isSel ? T9_RED : borderColor, background: isSel ? (isDark ? 'rgba(231,76,60,0.1)' : '#34d399') : 'transparent' }}>
                    ...
                    {(config.phone_numbers && config.phone_numbers.length > 0) && (
                        <div className="relative shrink-0">
                            <button
                                ...
                                className="w-10 h-10 rounded-full flex items-center justify-center bg-gray-100 dark:bg-slate-800 text-[#059669] hover:bg-red-50 dark:hover:bg-slate-700 transition-colors border border-gray-200 dark:border-slate-700 shadow-sm"
                            >
