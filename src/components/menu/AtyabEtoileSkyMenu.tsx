 ...
 */
export default function AtyabEtoileSkyMenu({ config, categories, language, restaurantId }: Props) {
    ...
    return (
        <div className="min-h-screen bg-white dark:bg-[#121212] text-[#333] dark:text-[#F5F5F5] antialiased" style={{ fontFamily: "'Tajawal', 'Cairo', sans-serif" }}>
            ...
            <header className="relative z-50 bg-[#FDFBF7] dark:bg-[#1E1E1E] shadow-sm pt-[env(safe-area-inset-top,0px)]">
                <div className="max-w-4xl mx-auto px-4 py-3 md:py-4">
                    ...
                    <div className="flex items-center justify-between mb-4">
                        {/* Hamburger */}
                        <button onClick={() => { haptic(); setShowCategoriesMenu(true); }} className="w-10 h-10 border border-[#38bdf8] dark:border-[#333] rounded-md flex items-center justify-center text-[#888] hover:text-[#0284c7] transition-colors active:scale-95">
                        ...
                        {/* Dark mode toggle */}
                        <button onClick={toggleDarkMode} className="w-10 h-10 rounded-full flex items-center justify-center text-lg text-[#888] hover:text-[#0284c7] hover:bg-[#0284c7]/5 transition-colors active:scale-95 border border-[#38bdf8] dark:border-[#333]" suppressHydrationWarning>
                            {isDark ? "☀️" : "🌙"}
                        </button>
                    </div>
                    ...
                    <div className="relative">
                        <input
                            ...
                            className="w-full bg-transparent border border-[#38bdf8] dark:border-[#333] rounded-full py-2.5 px-6 text-sm outline-none focus:border-[#0284c7] transition-colors text-right"
                            dir="rtl"
                        />
            ...
            <nav className="sticky top-0 z-40 bg-white/95 dark:bg-[#121212]/95 backdrop-blur-md border-b border-[#38bdf8] dark:border-[#333] shadow-sm mt-6">
                <div className="max-w-4xl mx-auto relative flex items-center">
                    <button onClick={() => scrollNav("right")} className="absolute right-0 z-10 w-12 h-full bg-gradient-to-l from-white dark:from-[#121212] to-transparent flex items-center justify-center text-[#888] hover:text-[#0284c7] transition-colors">
                        ...
                        {categories.map((cat) => (
                            <button
                                ...
                                className={`whitespace-nowrap px-4 py-2 rounded-full text-sm font-bold transition-colors ${activeSection === cat.id
                                    ...
                                    : "bg-[#FDFBF7] dark:bg-[#1E1E1E] text-[#888] hover:text-[#0284c7] border border-[#38bdf8] dark:border-[#333]"
                                    }`}
                    ...
                    <button onClick={() => scrollNav("left")} className="absolute left-0 z-10 w-12 h-full bg-gradient-to-r from-white dark:from-[#121212] to-transparent flex items-center justify-center text-[#888] hover:text-[#0284c7] transition-colors">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
                            {cat.items.map((item) => {
                                ...
                                return (
                                    <div
                                        key={item.id}
                                        className={`bg-[#FDFBF7] dark:bg-[#1E1E1E] rounded-3xl p-3 md:p-4 shadow-sm border border-[#38bdf8] dark:border-[#333] relative group cursor-pointer hover:shadow-md transition-shadow ${hasManyPrices ? 'col-span-2 flex flex-row gap-3' : 'flex flex-col'}`}
                                    >
                                        {/* Item Image */}
                                        <div className={`overflow-hidden relative shrink-0 ${hasManyPrices ? 'w-28 md:w-36 rounded-xl' : 'w-full h-32 md:h-40'}`} style={{ backgroundColor: '#FDFBF7' }}>
                                            ...
                                            <button
                                                ...
                                                className={`absolute top-2 right-2 w-8 h-8 bg-white/90 rounded-full flex items-center justify-center shadow-sm transition-colors active:scale-95 ${favorites.includes(item.id) ? "text-red-500" : "text-[#888] hover:text-[#0284c7]"}`}
                                            >
                                                ...
                                                {item.prices.length > 1 ? (
                                                    <div className="w-full space-y-1 mb-3">
                                                        {item.prices.map((price, idx) => {
                                                            ...
                                                            return (
                                                                <div key={idx} className="flex justify-between items-center w-full text-xs bg-gray-50 dark:bg-black/20 px-2 py-1 rounded-md border border-[#38bdf8] dark:border-[#333]">
                ...
                <footer className="mt-16 rounded-[2rem] text-white p-6 sm:p-8" style={{ backgroundColor: PRIMARY }}>
                    <div className="max-w-4xl mx-auto flex flex-col items-center text-center gap-8">
                        ...
                        <div className="flex gap-6">
                            {config.map_link && (
                                <a href={config.map_link} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full border border-white flex items-center justify-center hover:bg-white hover:text-[#0284c7] transition-colors">
                                    <MapPin className="w-5 h-5" />
                                </a>
                            )}
                            {config.whatsapp_number && (
                                <a href={`https://wa.me/${config.whatsapp_number.replace(/\+/g, "")}`} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full border border-white flex items-center justify-center hover:bg-white hover:text-[#0284c7] transition-colors">
                                    <FaWhatsapp className="w-5 h-5" />
                                </a>
                            )}
                            {config.facebook_url && (
                                <a href={config.facebook_url} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full border border-white flex items-center justify-center hover:bg-white hover:text-[#0284c7] transition-colors">
                                    <FaFacebook className="w-5 h-5" />
                                </a>
                            )}
                {config.instagram_url && (
                                <a href={config.instagram_url} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full border border-white flex items-center justify-center hover:bg-white hover:text-[#0284c7] transition-colors">
                                    <FaInstagram className="w-5 h-5" />
                                </a>
                            )}
                            {config.tiktok_url && (
                                <a href={config.tiktok_url} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full border border-white flex items-center justify-center hover:bg-white hover:text-[#0284c7] transition-colors">
            ...
            <nav className="fixed bottom-0 left-0 right-0 z-[60] px-4 pb-8 pt-2 md:hidden">
                <div className="w-[85vw] max-w-[310px] mx-auto etoile-glass border border-[#38bdf8] dark:border-[#333] rounded-full p-1.5 flex items-center justify-around shadow-lg relative">
                    {config.whatsapp_number && (
                    ...
                    <button onClick={() => { haptic(); setShowCallMenu(!showCallMenu); setShowCategoriesMenu(false); }} className={`flex-1 flex flex-col items-center py-2 active:scale-95 transition-all ${showCallMenu ? "text-[#0284c7]" : "text-[#888]"}`}>
                    ...
                    <button onClick={() => { setShowCategoriesMenu(true); setShowCallMenu(false); }} className={`flex-1 flex flex-col items-center py-2 active:scale-95 transition-all ${showCategoriesMenu ? "text-[#0284c7]" : "text-[#888]"}`}>
                    ...
                    <AnimatePresence>
                        {showCallMenu && (
                            <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }}
                                className="absolute bottom-[calc(100%+1.5rem)] left-0 right-0 bg-[#FDFBF7] dark:bg-[#1E1E1E] border border-[#38bdf8] dark:border-[#333] rounded-[1.5rem] shadow-xl mx-2 z-[62] overflow-hidden"
                            >
                                <div className="px-5 py-3 bg-white dark:bg-[#121212] border-b border-[#38bdf8] dark:border-[#333] text-right flex justify-between items-center flex-row-reverse">
                                ...
                                {config.phone_numbers?.map((p, i) => (
                                    <a key={i} href={`tel:${p.number}`} className="flex items-center justify-between px-6 py-4 border-b last:border-0 border-[#38bdf8] dark:border-[#333] active:bg-[#0284c7]/5 transition-colors group">
                            ...
                            <div className="p-4 grid grid-cols-2 gap-3 max-h-[85vh] overflow-y-auto etoile-no-scrollbar">
                                {categories.map((cat) => (
                                    <button
                                        key={cat.id}
                                        onClick={() => handleNavClick(cat.id)}
                                        className="flex flex-col items-center justify-center gap-2 bg-white dark:bg-[#121212] hover:bg-[#0284c7]/5 p-4 rounded-3xl border border-[#38bdf8] dark:border-[#333] active:scale-95 transition-all group"
                                    >
                                        <span className="text-3xl">{cat.emoji || "✨"}</span>
                                        <span className="text-[13px] font-semibold group-hover:text-[#0284c7] uppercase text-center leading-tight">
                                            {isAr ? cat.name_ar : cat.name_en || cat.name_ar}
                                        </span>
                                    </button>
                                ))}
                ...
                {selectedItem && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex bg-black/60 items-center justify-center backdrop-blur-md py-16 px-6 mb-safe" onClick={() => { setSelectedItem(null); setItemNotes(''); }}>
                        <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} className="w-[85vw] max-w-[310px] mx-auto bg-white dark:bg-[#1E1E1E] rounded-3xl shadow-2xl overflow-hidden  max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
                            ...
                            <div className="p-5 flex-1 overflow-y-auto">
                                ...
                                <div className="space-y-2">
                                    {selectedItem.item.prices.map((p, idx) => {
                                        ...
                                        return (
                                            <div key={idx} onClick={() => { haptic(5); setTempSizeIdx(idx); }} className={`flex justify-between items-center p-4 rounded-xl border-2 cursor-pointer transition-all ${isSelected ? "border-[#0284c7] bg-[#0284c7]/5" : "border-[#38bdf8] dark:border-[#333] hover:border-[#0284c7]/50"}`}>
                                                <div className="flex items-center gap-3">
                                                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${isSelected ? "border-[#0284c7]" : "border-gray-300"}`}>
                            ...
                            <div className="p-4 bg-gray-50 dark:bg-black/20 border-t border-[#38bdf8] dark:border-[#333]">
                ...
                {showCart && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex bg-black/60 items-center justify-center backdrop-blur-md py-16 px-6 mb-safe" onClick={() => setShowCart(false)}>
                        <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} className="w-[85vw] max-w-[310px] mx-auto bg-white dark:bg-[#1E1E1E] rounded-3xl shadow-2xl overflow-hidden flex flex-col h-auto max-h-[85vh]" onClick={e => e.stopPropagation()}>
                            <div className="p-4 border-b border-[#38bdf8] dark:border-[#333] flex justify-between items-center bg-gray-50 dark:bg-black/20">
                                ...
                                <h3 className="text-lg font-bold flex items-center gap-2">{isAr ? "سلة الطلبات" : "Your Cart"} <ShoppingCart className="w-5 h-5 text-[#0284c7]" /></h3>
                            </div>
                            <div className="p-5 overflow-y-auto flex-1">
                                {cart.length === 0 ? (
                                    ...
                                    <div className="space-y-4">
                                        {cart.map((c) => (
                                            <div key={c.id} className="flex gap-4 p-3 rounded-3xl border border-[#38bdf8] dark:border-[#333] bg-gray-50 dark:bg-black/10">
                                                <div className="flex flex-col items-center justify-between bg-white dark:bg-[#1E1E1E] rounded-full p-1 border border-[#38bdf8] dark:border-[#333]">
                                                    <button onClick={() => updateCartQty(c.id, 1)} className="w-7 h-7 rounded-full bg-gray-100 dark:bg-[#333] flex items-center justify-center hover:bg-[#0284c7] hover:text-white transition-colors" >+</button>
                                                ...
                                                <div className="flex-1 text-right flex flex-col justify-center">
                                                    <div className="flex justify-between items-start mb-1 flex-row-reverse">
                                                        ...
                                                        <span className="font-bold text-[#0284c7]">{c.price * c.quantity} {currency}</span>
                                                    </div>
                            ...
                            {cart.length > 0 && (
                                <div className="p-5 border-t border-[#38bdf8] dark:border-[#333] bg-gray-50 dark:bg-black/20">
                                    <div className="flex justify-between items-center mb-4">
                                        <span className="font-bold text-[#0284c7] text-xl">{cartTotal} {currency}</span>
                ...
                {showPaymentOptions && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        ...
                        <motion.div initial={{ scale: 0.95, opacity: 0, y: 50 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 50 }}
                            className="bg-white dark:bg-[#1E1E1E] w-[85vw] max-w-[310px] rounded-[2rem] shadow-2xl flex flex-col overflow-hidden max-h-[85vh] border border-[#38bdf8] dark:border-[#333] mx-auto"
                            onClick={(e) => e.stopPropagation()}>
                            <div className="p-6 border-b border-[#38bdf8] dark:border-[#333] flex items-center justify-between">
                            ...
                            <div className="p-4 overflow-y-auto flex flex-col gap-3">
                                {config.payment_methods?.map((pm) => (
                                    <div key={pm.id} className="bg-[#FDFBF7] dark:bg-[#121212] border border-[#38bdf8] dark:border-[#333] rounded-[1.5rem] p-4 text-right flex flex-col transition-all hover:border-[#0284c7]/50">
                                        ...
                                        {pm.number && (
                                            <div className="flex items-center justify-between bg-white dark:bg-[#1E1E1E] px-4 py-3 rounded-xl mb-3 border border-[#38bdf8] dark:border-[#333] group">
                                                <button
                                                    onClick={() => {
                                                        navigator.clipboard.writeText(pm.number!);
                                                        alert(isAr ? "تم نسخ الرقم!" : "Number copied!");
                                                    }}
                                                    className="text-[#0284c7] text-xs font-bold uppercase bg-[#0284c7]/10 px-3 py-1.5 rounded-lg active:scale-95"
                                                >
                                                    {isAr ? "نسخ" : "Copy"}
                                                </button>
