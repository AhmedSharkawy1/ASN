...
/* ───────────────────── THEME 6 CONSTANTS ───────────────────── */
const T6 = '#0284c7';        // brand-primary teal

/* ───────────────────── COMPONENT ───────────────────── */
export default function Theme6SkyMenu({ config, categories, restaurantId }: { config: any; categories: any[]; restaurantId: string }) {
                ...
                <div ref={catNavRef}
                    ...
                    <button onClick={scrollToTop} data-cat-id="top"
                        ...
                        <div className={`w-16 h-16 rounded-full flex items-center justify-center text-xl shadow-sm border-2 transition-colors
                            ${activeSection === '' ? 'border-[#0284c7] bg-[#38bdf8]' : 'border-transparent bg-zinc-100 dark:bg-zinc-800'}`}>
                            🏠
                        </div>
                        <span className={`text-sm font-bold transition-colors ${activeSection === '' ? 'text-[#0284c7]' : 'text-zinc-600 dark:text-zinc-400'}`}>
                            {isAr ? 'الكل' : 'All'}
                        </span>
                    </button>
                    ...
                    {categories.map((s: any) => (
                        <button key={s.id} data-cat-id={s.id}
                            ...
                            <span className={`text-sm font-bold transition-colors ${activeSection === s.id ? 'text-[#0284c7]' : 'text-zinc-600 dark:text-zinc-400'}`}>
                                                ...
                                                return (
                                                    <label key={idx}
                                                        className={`flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-colors
                                                               ${sel ? 'border-[#0284c7] bg-[#38bdf8]/50' : 'border-zinc-200 dark:border-zinc-700'}`}>
                                                        <div className="flex items-center gap-1">
