...
const THEME5_PRIMARY = '#0284c7'; // Light blue
const THEME5_LIGHT = '#38bdf8';
const THEME5_GREEN = '#a7f3d0';
const THEME5_GREEN_TEXT = '#059669';

export default function Theme5SkyMenu({ config, categories, language, restaurantId }: Props) {
                ...
                <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-100 dark:border-zinc-800 h-14">
                    <div className="flex items-center gap-4">
                        {isSearchOpen ? (
                            ...
                            <button onClick={() => setIsSearchOpen(true)} className="text-zinc-600 dark:text-zinc-300 transition-colors hover:text-[#0284c7]">
                                <Search className="w-5 h-5" />
                            </button>
                        )}
                        {!isSearchOpen && (
                            <button onClick={handleShare} className="text-zinc-600 dark:text-zinc-300 transition-colors hover:text-[#0284c7]">
                            ...
                            <div className="p-6 overflow-y-auto">
                                <div className="space-y-3">
                                    {selectedItem.item.prices.map((price, idx) => {
                                        ...
                                        return (
                                            <div key={idx} onClick={() => setTempSizeIdx(idx)} className={`flex justify-between items-center p-4 rounded-xl border-2 cursor-pointer flex-row-reverse ${isSelected ? 'border-[#0284c7] bg-[#38bdf8] dark:bg-[#0284c7]/10' : 'border-zinc-200 dark:border-zinc-800'}`}>
                                                <div className="flex items-center gap-3 flex-row-reverse">
                                                    ...
                                                    <span className={`font-bold ${isSelected ? 'text-[#0284c7]' : ''}`}>{label}</span>
                                                </div>
                                                <span className={`font-bold ${isSelected ? 'text-[#0284c7]' : ''}`}>{price} {currency}</span>
                                            </div>
                                        );
                                    })}
                                </div>
                                ...
                                <div className="mt-4">
                                    <textarea
                                        ...
                                        className="w-full p-3 rounded-xl border-2 border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 text-sm text-zinc-800 dark:text-zinc-200 placeholder-zinc-400 dark:placeholder-zinc-500 outline-none focus:border-[#0284c7] transition-colors resize-none"
                                        rows={2}
                                        dir={isAr ? "rtl" : "ltr"}
                                    />
                                </div>
                                        ...
                                        {cart.map((c) => (
                                            <div key={c.id} className="flex gap-4 p-3 border rounded-3xl flex-row-reverse border-zinc-100 dark:border-zinc-800 bg-gray-50 dark:bg-black/10">
                                                <div className="flex flex-col items-center justify-between bg-white dark:bg-[#1E1E1E] border rounded-full p-1 border-zinc-100 dark:border-zinc-800">
                                                    <button onClick={() => updateCartQty(c.id, 1)} className="w-7 h-7 rounded-full bg-zinc-100 dark:bg-[#333] flex items-center justify-center hover:bg-[#0284c7] hover:text-white transition-colors"><PlusIcon className="w-4 h-4" /></button>
                            ...
                            <div className="px-5 py-4 space-y-3 overflow-y-auto t5-no-scrollbar">
                                {config.payment_methods && config.payment_methods.length > 0 ? (
                                    config.payment_methods.map((pm) => (
                                        <div key={pm.id} className="block p-4 bg-zinc-50 dark:bg-zinc-800 rounded-3xl border border-zinc-100 dark:border-white/5 text-right transition-all hover:border-[#0284c7]/50">
