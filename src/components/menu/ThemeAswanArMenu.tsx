...
import { ShoppingCart, Plus, Minus, Trash2, X, Search, Share2, Home, Tag, Moon, Sun, LayoutGrid, LayoutList, CreditCard, ArrowLeft, Check, Phone, MapPin, Clock } from 'lucide-react';
...
import AswanArLandingPage from './AswanArLandingPage';
...
interface ThemeAswanArMenuProps {
    config: RestaurantType;
    categories: CategoryWithItemsType[];
    restaurantId: string;
}

export default function ThemeAswanArMenu({ config, categories, restaurantId }: ThemeAswanArMenuProps) {
    ...
    const cur = parseCurrency(config?.currency, true); // Force Arabic currency string (ج.م)
    ...
    const handleShare = async () => {
        ...
        if (navigator.share) {
            ...
            alert('تم نسخ رابط القائمة بالحافظة!');
        }
    };
    ...
    if (!inMenu && landingEnabled) {
        return <AswanArLandingPage config={config} onContinue={() => setInMenu(true)} />;
    }
            ...
            <div className="relative z-10">
                ...
                {config.marquee_enabled && (
                    <div className="text-xs md:text-sm text-white py-1.5 font-semibold tracking-normal" style={{ backgroundColor: primaryColor }}>
                        <SharedMarquee text={config.marquee_text_ar || config.marquee_text_en || 'أهلاً بكم في مطعمنا! استمتعوا بأشهى المأكولات والمشروبات.'} direction="rtl" />
                    </div>
                )}
                ...
                <div className="px-5 pt-6 pb-4 max-w-4xl mx-auto">
                    ...
                    <div className="relative max-w-4xl mx-auto mt-2">
                        ...
                        <input 
                            ...
                            className="w-full h-12 rounded-2xl pr-12 pl-4 outline-none font-medium text-sm shadow-md border transition-all focus:ring-2"
                            style={{ backgroundColor: bgCard, color: textMain, borderColor: borderColor }}
                        />
                        {searchQuery && (
                            <button onClick={() => setSearchQuery('')} className="absolute inset-y-0 left-4 flex items-center">
                    ...
                    <div className="px-5 mb-6 max-w-4xl mx-auto">
                        <Swiper modules={[Autoplay]} autoplay={{ delay: 3500, disableOnInteraction: false }} className="w-full rounded-3xl overflow-hidden shadow-lg border" style={{ borderColor: borderColor }}>
                            {config.cover_images.map((img, i) => (
                                <SwiperSlide key={i}>
                                    <OptimizedMenuImage src={img} alt="العرض" className="w-full h-[160px] md:h-[220px] object-cover" />
                                </SwiperSlide>
                            ))}
                        </Swiper>
                    </div>
                    ...
                    <div className="mb-8 max-w-4xl mx-auto">
                        <div className="px-5 flex items-center justify-between mb-3">
                            <div className="flex items-center gap-2">
                                ...
                                <h2 className="text-lg font-bold">العروض المميزة</h2>
                            </div>
                        </div>
                        ...
                        >
                            ...
                            >
                                <div className="relative h-48 md:h-56 w-full bg-slate-900 shrink-0">
                                    ...
                                    <button 
                                        onClick={closeModal}
                                        className="absolute top-3 left-3 w-9 h-9 rounded-full bg-black/60 text-white flex items-center justify-center backdrop-blur-md transition-transform active:scale-95 shadow-md"
                                    >
                                        <X className="w-5 h-5" />
                                    </button>
                                </div>
                                ...
                                <div className="p-5 overflow-y-auto space-y-5 flex-1">
                                    ...
                                    {selectedItem.item.prices && selectedItem.item.prices.length > 1 && (
                                        <div className="space-y-2.5">
                                            <label className="font-bold text-xs uppercase tracking-wider block opacity-70">اختر الحجم</label>
                                            <div className="grid grid-cols-2 gap-2">
                                                {selectedItem.item.prices.map((p, idx) => (
                                                    ...
                                                    >
                                                        <span>{selectedItem.item.size_labels?.[idx] || `خيار ${idx + 1}`}</span>
                                    ...
                                    {selectedItem.item.extras && selectedItem.item.extras.length > 0 && (
                                        <div className="space-y-2.5">
                                            <label className="font-bold text-xs uppercase tracking-wider block opacity-70">الإضافات والخيارات</label>
                                            <div className="space-y-2">
                                                {selectedItem.item.extras.map((ext, idx) => {
                                                    const extName = ext.name_ar || ext.name_en || 'إضافة';
                                    ...
                                    <div className="space-y-1.5">
                                        <label className="font-bold text-xs uppercase tracking-wider block opacity-70">ملاحظات خاصة</label>
                                        <textarea
                                            value={notes}
                                ...
                                <div className="p-4 border-t flex items-center gap-3 shrink-0" style={{ borderColor: borderColor }}>
                                    ...
                                    <button
                                        ...
                                        style={{ backgroundColor: primaryColor }}
                                    >
                                        <span className="whitespace-nowrap">إضافة إلى السلة</span>
                                        <span className="font-extrabold whitespace-nowrap mr-1.5">
                    ...
                    <div className="fixed bottom-6 inset-x-5 max-w-md mx-auto z-40">
                        ...
                        >
                            <div className="flex items-center gap-3">
                                ...
                                <span className="font-extrabold text-lg">عرض السلة</span>
                            </div>
                            ...
                            >
                                <div className="p-4 border-b flex justify-between items-center shrink-0" style={{ borderColor: borderColor }}>
                                    <div className="flex items-center gap-2">
                                        ...
                                        <h3 className="font-bold text-lg">سلة طلباتك ({cartCount})</h3>
                                    </div>
                        ...
                        >
                            ...
                            >
                                ...
                                <div className="space-y-4">
                                    {displayNumbers.length > 0 && (
                                        <div className="space-y-2">
                                            <span className="text-xs font-bold uppercase tracking-wider block opacity-70">أرقام الاتصال المباشر</span>
                                            <div className="space-y-2">
                                    ...
                                    {config.address && (
                                        <div className="p-3.5 rounded-2xl border flex items-start gap-3" style={{ borderColor: borderColor, backgroundColor: bgBody }}>
                                            ...
                                            <div>
                                                <h4 className="font-bold text-xs uppercase opacity-70">العنوان والموقع</h4>
                                    ...
                                    {config.working_hours && (
                                        <div className="p-3.5 rounded-2xl border flex items-start gap-3" style={{ borderColor: borderColor, backgroundColor: bgBody }}>
                                            ...
                                            <div>
                                                <h4 className="font-bold text-xs uppercase opacity-70">ساعات العمل</h4>
                            ...
                            >
                                <div className="flex justify-between items-center mb-3">
                                    <h3 className="font-bold text-base flex items-center gap-2">
                                        ...
                                        <span>خيارات الدفع والتحويل</span>
                                    </h3>
                                ...
                                {config.whatsapp_number && (
                                    <a
                                        href={`https://wa.me/${config.whatsapp_number.replace(/\+/g, '')}?text=${encodeURIComponent('مرحباً، لقد قمت بإتمام عملية التحويل المالي. إليكم صورة الإيصال:')}`}
                                        target="_blank"
                                        rel="noopener noreferrer"
                                    ...
                                    >
                                        <FaWhatsapp className="w-5 h-5" />
                                        <span>إرسال إيصال التحويل عبر الواتساب</span>
                                    </a>
                                )}
                                ...
                                <div className="space-y-3">
                                    {config.payment_methods?.map((pm: any, idx: number) => (
                                        <div key={idx} className="p-3.5 rounded-2xl border space-y-2" style={{ borderColor: borderColor, backgroundColor: bgBody }}>
                                            <h4 className="font-bold text-sm">{pm.name_ar || pm.name_en}</h4>
                                            {(pm.desc_ar || pm.desc_en) && <p className="text-xs opacity-70 font-normal">{pm.desc_ar || pm.desc_en}</p>}
                                            {pm.number && (
