    ...
    return (
        <div className="min-h-screen bg-white dark:bg-[#121212] text-[#333] dark:text-[#F5F5F5] antialiased" style={{ fontFamily: "'Tajawal', 'Cairo', sans-serif" }}>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
                            {cat.items.map((item) => {
                                ...
                                return (
                                    <div
                                        key={item.id}
                                        className={`bg-[#FDFBF7] dark:bg-[#1E1E1E] rounded-2xl p-3 md:p-4 shadow-sm border border-[#34d399] dark:border-[#333] relative group cursor-pointer hover:shadow-md transition-shadow ${hasManyPrices ? 'col-span-2 flex flex-row gap-3' : 'flex flex-col'}`}
                                    >
                                        {/* Item Image */}
            ...
            <nav className="fixed bottom-0 left-0 right-0 z-[60] px-4 pb-8 pt-2 md:hidden">
                <div className="max-w-xl mx-auto etoile-glass border border-[#34d399] dark:border-[#333] rounded-full p-1.5 flex items-center justify-around shadow-lg relative">
                    {config.whatsapp_number && (
                    ...
                    <AnimatePresence>
                        {showCallMenu && (
                            ...
                            >
                                ...
                                {config.phone && (
                                    <a href={`tel:${config.phone}`} className="flex items-center justify-between px-6 py-4 border-b border-[#34d399] dark:border-[#333] active:bg-[#059669]/5 transition-colors group">
                                        <span className="text-[12px] font-medium text-[#888] group-hover:text-[#059669]">{isAr ? "الرئيسي" : "Main"}</span>
                ...
                {showCategoriesMenu && (
                    ...
                    >
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
                            className="w-full max-w-sm bg-[#FDFBF7] dark:bg-[#1E1E1E] rounded-[2rem] shadow-2xl border border-[#34d399] dark:border-[#333] overflow-hidden"
                            ...
                            <div className="p-4 grid grid-cols-2 gap-3 max-h-[60vh] overflow-y-auto etoile-no-scrollbar">
                                {categories.map((cat) => (
                                    <button
                                        key={cat.id}
                                        onClick={() => handleNavClick(cat.id)}
                                        className="flex flex-col items-center justify-center gap-2 bg-white dark:bg-[#121212] hover:bg-[#059669]/5 p-4 rounded-2xl border border-[#34d399] dark:border-[#333] active:scale-95 transition-all group"
                                    >
                ...
                {showCart && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex flex-col justify-end md:justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setShowCart(false)}>
                        <motion.div initial={{ y: 200, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 200, opacity: 0 }} className="w-full max-w-lg mx-auto bg-white dark:bg-[#1E1E1E] rounded-3xl shadow-2xl overflow-hidden mt-auto md:mt-0 flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                            <div className="p-5 border-b border-[#34d399] dark:border-[#333] flex justify-between items-center bg-gray-50 dark:bg-black/20">
                                ...
                                <h3 className="text-xl font-bold flex items-center gap-2">{isAr ? "سلة الطلبات" : "Your Cart"} <ShoppingCart className="w-5 h-5 text-[#059669]" /></h3>
                            </div>
                            <div className="p-5 overflow-y-auto flex-1">
                                {cart.length === 0 ? (
                                    ...
                                    <div className="space-y-4">
                                        {cart.map((c) => (
                                            <div key={c.id} className="flex gap-4 p-3 rounded-2xl border border-[#34d399] dark:border-[#333] bg-gray-50 dark:bg-black/10">
                ...
                {showPaymentOptions && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        ...
                        <motion.div initial={{ scale: 0.95, opacity: 0, y: 50 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 50 }}
                            className="bg-white dark:bg-[#1E1E1E] w-full max-w-sm rounded-[2rem] shadow-2xl flex flex-col overflow-hidden max-h-[85vh] border border-[#34d399] dark:border-[#333]"
                            onClick={(e) => e.stopPropagation()}>
