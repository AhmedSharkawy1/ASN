...
import { 
    FaWhatsapp, FaFacebookF, FaSnapchatGhost, FaInstagram, 
    FaTiktok, FaMapMarkerAlt, FaPhoneAlt, FaYoutube 
} from 'react-icons/fa';
...
interface LametZamanCompactMenuProps {
...
export default function LametZamanCompactMenu({ config, categories, restaurantId, suppressInternalPopup = false }: LametZamanCompactMenuProps) {
    ...
    const primaryColor = getPrimaryColor(config?.theme, config?.theme_colors?.primary);
    ...
    const bgCard = hasBgImage ? (isDark ? 'rgba(28, 28, 30, 0.92)' : 'rgba(255, 255, 255, 0.95)') : (isDark ? '#1c1c1e' : '#ffffff');
    ...
    const phoneList: { label: string; number: string }[] = (() => {
        ...
        if (Array.isArray(config?.phone_numbers) && config.phone_numbers.length > 0) {
            config.phone_numbers.forEach((p: any, idx: number) => {
                const totalPhones = config.phone_numbers?.length ?? 0;
                if (typeof p === 'object' && p?.number) {
                    list.push({
                        label: p.label || (isAr ? (totalPhones > 1 ? `رقم التواصل ${idx + 1}` : 'رقم الدليفري') : (totalPhones > 1 ? `Contact ${idx + 1}` : 'Delivery Number')),
                        number: String(p.number).trim()
                    });
                } else if (typeof p === 'string' && p.trim()) {
                    list.push({
                        label: isAr ? (totalPhones > 1 ? `رقم التواصل ${idx + 1}` : 'رقم الدليفري') : (totalPhones > 1 ? `Contact ${idx + 1}` : 'Delivery Number'),
                        number: p.trim()
                    });
                }
            });
    ...
    // Default viewMode for Theme 27 is initialized from dashboard settings (or falls back to 'compact')
    const initialViewMode = (tc.theme27_default_view_mode || config?.theme27_default_view_mode || 'compact') as 'compact' | 'grid' | 'list' | 'single';
    const validViewMode = (['compact', 'grid', 'list', 'single'].includes(initialViewMode)) ? initialViewMode : 'compact';
        ...
        const handleScroll = () => {
            ...
            if (window.scrollY < 180) {
                if (activeCategory !== 'all') {
                    ...
                    document.getElementById('compact-nav-cat-all')?.scrollIntoView({ behavior: 'smooth', inline: 'center', block: 'nearest' });
                }
                return;
            }
            ...
            if (newActive !== activeCategory) {
                ...
                document.getElementById(`compact-nav-cat-${newActive}`)?.scrollIntoView({ behavior: 'smooth', inline: 'center', block: 'nearest' });
            }
        };
    ...
    // Helpers
    const itemName = (item: MenuItem) => (isAr ? (item.title_ar || item.title_en) : (item.title_en || item.title_ar)) || '';
    const catName = (cat: CategoryWithItemsType) => (isAr ? (cat.name_ar || cat.name_en) : (cat.name_en || cat.name_ar)) || '';
    
    // Get item ingredients or description
    const getItemDescription = (item: MenuItem) => {
        if (isAr) {
            return item.ingredients_ar || item.ingredients || item.description_ar || item.desc_ar || '';
        }
        ...
        if (navigator.share) {
            try {
                await navigator.share({
                    title: config.name,
                    text: isAr ? `تصفح منيو ${config.name}` : `Check out the menu of ${config.name}`,
    ...
    const searchedCategories = categories.map(cat => ({
        ...
        items: (cat.items || []).filter(item => {
            ...
            const nameMatch = itemName(item).toLowerCase().includes(query);
        ...
        >
            ...
            {hasBgImage && (
                <div 
                    ...
                    style={{ backgroundColor: isDark ? 'rgba(15, 23, 42, 0.45)' : 'rgba(255, 255, 255, 0.35)' }}
                />
            )}
            ...
            <div className="relative z-10">
                ...
                {/* --- HEADER --- */}
                <div className="px-5 pt-7 pb-3" style={{ backgroundColor: hasBgImage ? 'transparent' : bgBody }}>
                    ...
                    {/* Language Toggle */}
                    <div className="flex justify-center items-center gap-2 mb-5 bg-black/5 dark:bg-white/5 w-fit mx-auto rounded-full p-1" dir="ltr">
                        <button 
                    ...
                    <div className="relative mb-4">
                        ...
                        <input 
                            type="text" 
                            placeholder={isAr ? 'ابحث في المنيو (بالاسم أو المكونات)...' : 'Search in menu (name or ingredients)...'} 
                            value={searchQuery}
                            onChange={e => setSearchQuery(e.target.value)}
                            className="w-full h-11 sm:h-12 rounded-full pr-12 pl-4 outline-none font-bold shadow-sm text-sm"
                ...
                {config.cover_images && config.cover_images.length > 0 && !searchQuery && (
                    <div className="px-5 mb-5 max-w-4xl mx-auto">
                        <Swiper key={isAr ? 'rtl' : 'ltr'} modules={[Autoplay]} autoplay={{ delay: 3500, disableOnInteraction: false }} className="w-full rounded-3xl overflow-hidden shadow-lg border" style={{ borderColor: borderColor }}>
                            {config.cover_images.map((img, i) => (
                                <SwiperSlide key={i}>
                                    <OptimizedMenuImage src={img} alt="Offer" className="w-full h-[150px] md:h-[210px] object-cover" highQuality={Boolean((config as any)?.high_quality_images)} />
                                </SwiperSlide>
                            ))}
                        </Swiper>
                    </div>
                ...
                {!searchQuery && featuredItems.length > 0 && (
                    <div className="mb-6">
                        <div className="px-5 flex items-center gap-2 mb-3">
                            <h2 className="text-lg sm:text-xl font-bold">{isAr ? 'عروض مميزة' : 'Featured Offers'}</h2>
                            <Tag className="w-5 h-5" style={{ color: primaryColor }} />
                        </div>
                        <div className="pr-5 overflow-hidden" dir={isAr ? "rtl" : "ltr"}>
                            <Swiper key={isAr ? 'rtl' : 'ltr'} spaceBetween={12} slidesPerView={1.4} breakpoints={{ 640: { slidesPerView: 2.3 }, 1024: { slidesPerView: 3.3 } }} className="w-full" modules={[Autoplay]} autoplay={{ delay: 2500, disableOnInteraction: false }} loop={featuredItems.length > 2}>
                                    ...
                                    <SwiperSlide key={idx} className="pb-3">
                                        ...
                                        >
                                            <div className="relative h-[135px] sm:h-[150px]">
                                                ...
                                                <div className="absolute top-2.5 left-2.5 bg-red-600/90 backdrop-blur-sm text-white px-2.5 py-0.5 rounded-full text-xs font-bold flex items-center gap-1">
                                                    ...
                                                    {item.prices && item.prices.length > 1 ? (
                                                            ...
                                                            {item.prices.map((price, pIdx) => {
                                                                ...
                                                                return (
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <div className="flex items-center justify-center gap-1 mt-0.5 leading-none flex-wrap" dir="ltr">
                                                                            {oldPrice && Number(oldPrice) > Number(price) ? (
                                                                                <span className="line-through text-red-500 dark:text-red-400 font-bold text-[8px] sm:text-[9px] decoration-red-500 decoration-1">
                                                                                    {oldPrice}
                                                                                </span>
                                                                            ) : null}
                                                                            <span style={{ color: primaryColor }} className="font-black text-[10.5px] sm:text-[11px] whitespace-nowrap">{price}</span>
                                                                            <span className="text-[7.5px] sm:text-[8px] font-bold opacity-60 whitespace-nowrap">{cur}</span>
                                                                        </div>
                                                                    </div>
                                                                );
                                                            })}
                                                        ...
                                                        <div className="flex items-center justify-between w-full" dir="ltr">
                                                            ...
                                                            <div className="flex items-center gap-1.5 shrink-0 ml-1">
                                                                {item.old_prices?.[0] && Number(item.old_prices[0]) > Number(item.prices?.[0] || 0) ? (
                                                                    <span className="text-[10.5px] sm:text-[11px] line-through text-red-500 dark:text-red-400 font-bold decoration-red-500 decoration-1">
                ...
                <div className="px-2.5 sm:px-4">
                    ...
                    {/* View Mode Switcher with responsive mobile grid */}
                    {!searchQuery && (
                        <div className="flex items-center justify-between gap-1.5 mb-2.5 px-0.5 w-full">
                            <span className="text-[11px] sm:text-xs font-black opacity-80 shrink-0">
                                {isAr ? 'العرض:' : 'View:'}
                            </span>
                            <div className="grid grid-cols-4 sm:flex items-center gap-0.5 p-0.5 bg-black/5 dark:bg-white/5 rounded-xl border border-black/5 dark:border-white/10 flex-1 sm:flex-none max-w-[285px] sm:max-w-none">
                                {/* Compact Rows (Signature Theme 27 option) */}
                                <button
                                    onClick={() => setViewMode('compact')}
                                    title={isAr ? "عرض مكثف أفقي" : "Dense Horizontal Rows"}
                                    className={`flex items-center justify-center gap-0.5 sm:gap-1 py-1 px-1 sm:px-2 rounded-lg text-center transition-all ${
                                        viewMode === 'compact' 
                                            ? 'bg-white dark:bg-zinc-800 shadow-sm font-black' 
                                            : 'opacity-60 hover:opacity-100 font-medium'
                                    }`}
                                    style={viewMode === 'compact' ? { color: primaryColor } : {}}
                                >
                                    <AlignJustify className="w-3 h-3 sm:w-3.5 sm:h-3.5 shrink-0" />
                                    <span className="text-[9px] sm:text-[10px] truncate">{isAr ? "مكثف" : "Compact"}</span>
                                </button>
                                ...
                                >
                                    ...
                                    <span className="text-[9px] sm:text-[10px] truncate">{isAr ? "صورتين" : "Grid"}</span>
                                </button>
                                ...
                                >
                                    ...
                                    <span className="text-[9px] sm:text-[10px] truncate">{isAr ? "قائمة" : "List"}</span>
                                </button>
                                ...
                                >
                                    ...
                                    <span className="text-[9px] sm:text-[10px] truncate">{isAr ? "كبيرة" : "Large"}</span>
                                </button>
                            </div>
                        </div>
                    )}
                        ...
                        {displayCategories.map((category) => {
                            ...
                            return (
                                <div key={category.id} id={`cat-${category.id}`} className="mb-3.5 pt-0.5">
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-lg overflow-hidden shrink-0 relative bg-black/5 dark:bg-white/5 self-center">
                                                            ...
                                                            />
                                                            {Boolean(item.is_popular || (item.old_prices && item.old_prices.some((op, idx) => op && item.prices?.[idx] && Number(op) > Number(item.prices[idx])))) && (
                                                                <div className="absolute top-0.5 right-0.5 bg-red-600 text-white rounded-full font-bold px-1.5 py-0.5 text-[8px] shadow-sm">
                                                                    {isAr ? 'عرض' : 'Offer'}
                                                                </div>
                                                            )}
                                                        </div>
                                                            ...
                                                            {hasMultipleSizes ? (
                                                                ...
                                                                <div className="flex items-center gap-1.5 mt-0.5" dir="ltr">
                                                                    {Boolean(item.old_prices?.[0] && Number(item.old_prices[0]) > Number(item.prices?.[0])) && (
                                                                        <span className="line-through text-red-500 font-bold decoration-red-500 text-[10.5px] sm:text-xs">
                                                                            {item.old_prices?.[0]} {cur}
                                                                        </span>
                                                                    )}
                                    ...
                                    {/* 2. GRID / LIST / SINGLE MODES (Compatible with Lamet Zaman Classic) */}
                                    {viewMode !== 'compact' && (
                                        <div className={
                                            viewMode === 'single'
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <div className={`relative shrink-0 ${
                                                            ...
                                                            />
                                                            {Boolean(item.is_popular || (item.old_prices && item.old_prices.some((op, idx) => op && item.prices?.[idx] && Number(op) > Number(item.prices[idx])))) && (
                                                                <div className={`absolute top-2 right-2 bg-red-600 backdrop-blur-sm text-white rounded-full font-bold shadow-sm ${
                                ...
                                <div className="flex-1 overflow-y-auto">
                                    ...
                                    <div className="p-6 -mt-8 relative rounded-t-[2rem]" style={{ backgroundColor: bgBody }}>
                                        ...
                                        <div className="flex flex-col mb-4">
                                            <h2 className="text-2xl sm:text-3xl font-black mb-2 leading-tight">{itemName(selectedItem.item)}</h2>
                                            {getItemDescription(selectedItem.item) && (
                                                <div className="mb-5 p-3.5 rounded-2xl bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/10">
                                                    <span className="text-xs font-bold block mb-1 opacity-60">
                                                        {isAr ? 'المكونات والتفاصيل:' : 'Ingredients & Details:'}
                                                    </span>
                                                        ...
                                                        <div className="flex gap-2 items-center">
                                                            {Boolean(selectedItem.item.old_prices?.[sizeIdx] && Number(selectedItem.item.old_prices[sizeIdx]) > Number(selectedItem.item.prices?.[sizeIdx])) && (
                                                                <span className="text-base sm:text-lg line-through text-red-500 font-bold decoration-red-500">
                                                                    {Number(selectedItem.item.old_prices?.[sizeIdx] || 0) + extrasUnitTotal} {cur}
                                                                </span>
                                                            )}
                                                        ...
                                                        return (
                                                            <button
                                                                ...
                                                                style={{
                                                                    ...
                                                                    backgroundColor: isSelected ? `${primaryColor}15` : 'rgba(0,0,0,0.03)'
                                                                }}
                                                            >
                                            ...
                                            <div className="mb-6">
                                                ...
                                                {availableAddons.length > 4 && (
                                                    <button
                                                        ...
                                                        className="mt-2 text-xs font-bold w-full py-1.5 text-center rounded-xl bg-black/5 dark:bg-white/5 hover:opacity-80 transition-opacity"
                                                        style={{ color: primaryColor }}
                                                    >
                                                        {showAllAddons 
                                                            ? (isAr ? 'عرض أقل' : 'Show less') 
                                                            : (isAr ? `عرض المزيد من الإضافات (${availableAddons.length - 4})` : `Show more add-ons (${availableAddons.length - 4})`)}
                                                    </button>
                                                )}
                                            </div>
                                        )}
                                        ...
                                        {featuredItems.length > 0 && (
                                            <div>
                                                <div className="flex items-center gap-2 mb-3">
                                                    <h3 className="font-bold text-base sm:text-lg">{isAr ? 'قد يعجبك أيضاً' : 'You might also like'}</h3>
                                                </div>
                                                <div className="overflow-hidden" dir={isAr ? "rtl" : "ltr"}>
                                                    <Swiper key={isAr ? 'rtl' : 'ltr'} spaceBetween={12} slidesPerView={2.2} className="w-full pb-3">
                                                        {featuredItems.map((item, idx) => (
                                                            <SwiperSlide key={idx}>
                                                                <div 
                                                                ...
                                                                >
                                                                    <div className="relative h-[110px]">
                                                                        <OptimizedMenuImage thumbnailSrc={item.thumbnail_url} originalSrc={item.image_url || item.image || "https://images.unsplash.com/photo-1546069901-ba9599a7e63c"} alt={itemName(item)} className="w-full h-full object-cover" highQuality={Boolean((config as any)?.high_quality_images)} />
                                                                        {item.is_popular && (
                                                                            <div className="absolute top-2 left-2 bg-red-600/90 text-white px-1.5 py-0.5 rounded-full text-[9px] font-bold">
                                                                                {isAr ? 'عرض' : 'Offer'}
                                                                            </div>
                                                                        )}
                                                                    ...
                                                                    <div className="p-2 text-center">
                                                                        ...
                                                                        <div className="flex flex-col gap-0.5 mt-auto w-full" dir="ltr">
                                                                            <span className="font-black text-[11px]" style={{ color: primaryColor }}>{item.prices?.[0]} {cur}</span>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </SwiperSlide>
                                            ...
                                            <button
                                                ...
                                                onClick={() => {
                                                    ...
                                                    setTimeout(() => {
                                                        ...
                                                        const navEl = document.getElementById(`compact-nav-cat-${catIdStr}`);
                                                        if (navEl) {
