...
export default function PizzaPastaEmeraldMenu({ config, categories, language, restaurantId }: Props) {
    ...
    return (
        <div className="min-h-screen bg-zinc-50 dark:bg-[#050505] text-zinc-900 dark:text-zinc-200 antialiased selection:bg-emerald-500/30 font-cairo" dir="rtl">
                ...
                <div className="max-w-2xl mx-auto px-5 py-4 md:py-6 text-right">
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-3 group">
                            {config.logo_url && (
                                <div className="w-16 h-16 relative flex items-center justify-center overflow-hidden rounded-full border-2 border-emerald-600 shadow-xl bg-white dark:bg-zinc-900 p-1 group-hover:rotate-12 transition-transform duration-500">
                                    <OptimizedMenuImage src={config.logo_url} alt="Logo" className="w-full h-full object-contain rounded-full" useOriginal={true} />
                                    <div className="absolute inset-0 bg-emerald-500/20 blur-xl rounded-full -z-10 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                                </div>
                            )}
                            <div className="flex flex-col">
                                ...
                                <span className="text-emerald-600 dark:text-emerald-500 text-[10px] font-black uppercase tracking-[0.1em] mt-1">
                        ...
                        {categories.map((cat) => (
                            <button
                                ...
                                className={`whitespace-nowrap px-4 py-2 rounded-3xl text-[12px] font-black border transition-all duration-300 ${activeSection === cat.id
                                    ? "bg-emerald-600 text-white border-emerald-500 scale-105 shadow-md"
                ...
                <AnimatePresence>
                    {cart.length > 0 && !showCart && !selectedItem && (
                        <motion.button
                            ...
                            className="fixed bottom-24 right-6 left-6 z-[70] bg-emerald-600 text-white p-5 rounded-3xl shadow-2xl flex items-center justify-between animate-reveal border border-white/20 hover:scale-[1.02] transition-transform"
                        >
                            <div className="flex items-center gap-3">
                                <div className="bg-white text-emerald-600 w-7 h-7 rounded-xl flex items-center justify-center font-black text-sm shadow-inner animate-popular">{cart.length}</div>
                    ...
                    <section key={cat.id} id={cat.id.toString()} className="mb-6 scroll-mt-[170px]">
                        ...
                        <div className="relative aspect-[16/10] md:aspect-[21/9] rounded-[2rem] overflow-hidden mb-4 shadow-2xl border border-zinc-200 dark:border-white/5 bg-zinc-200 dark:bg-zinc-900 group">
                            {cat.image_url ? (
                                ...
                                <div className="w-full h-full bg-gradient-to-br from-emerald-600/30 via-zinc-900 to-emerald-900/50 flex items-center justify-center">
                            ...
                            <div className="absolute bottom-6 right-6 left-6 text-right">
                                <div className="flex flex-col gap-1">
                                    <span className="text-emerald-500 font-black text-[10px] tracking-[0.2em] uppercase drop-shadow-md">{isAr ? "فئة القائمة" : "Menu Category"}</span>
                        ...
                        <div className="bg-white dark:bg-zinc-900/60 backdrop-blur-xl rounded-[2rem] p-4 md:p-6 border border-zinc-200 dark:border-white/10 shadow-xl">
                            <div className="mb-4 pb-3 border-b border-zinc-100 dark:border-white/5 text-center">
                                <p className="text-[10px] font-black text-emerald-600 dark:text-emerald-500 uppercase tracking-widest bg-emerald-50/50 dark:bg-emerald-900/10 py-2.5 rounded-xl border border-emerald-100 dark:border-emerald-900/20">
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`py-3 group transition-all -mx-2 px-2 rounded-3xl border border-transparent flex gap-3
                                                ...
                                                hover:bg-emerald-50/30 dark:hover:bg-emerald-900/5 hover:border-emerald-100 dark:hover:border-emerald-900/10`}
                                        >
                                            {/* Item Info */}
                                            <div className={`flex flex-col gap-0.5 flex-1 min-w-0 text-right ${hasManyPrices ? "w-full pr-0" : "pr-1"}`}>
                                                <div className="flex flex-wrap items-center justify-end gap-2">
                                                    ...
                                                    <span className="text-zinc-900 dark:text-zinc-100 font-black text-lg leading-tight group-hover:text-emerald-600 transition-colors order-2 text-right w-full">
                                                    ...
                                                    config.orders_enabled !== false ? (
                                                        <button
                                                            ...
                                                            className={`flex flex-col items-center gap-1 transition-all group/btn
                                                                ${hasManyPrices
                                                                    ? "bg-zinc-50 dark:bg-white/5 border border-zinc-200 dark:border-white/10 rounded-3xl py-2 px-1 active:scale-95 hover:border-emerald-500/50"
                                                                    : "active:scale-90"
                                                                }`}
                                                        >
                                                            ...
                                                            <div className={`flex items-center justify-center gap-0.5
                                                                ${hasManyPrices ? "" : "bg-zinc-100 dark:bg-zinc-800/80 px-3 py-2 rounded-3xl border border-zinc-200 dark:border-white/5 hover:border-emerald-500/40 shadow-sm"}`}
                                                            >
                                                                <span className={`${hasManyPrices ? "text-emerald-600 dark:text-emerald-500 text-lg" : "text-emerald-600 text-base"} font-black leading-none tabular-nums`}>{price}</span>
                ...
                <footer className="mt-12 py-16 border-t border-zinc-200 dark:border-white/10 text-center space-y-10 pb-44">
                    {config.logo_url && (
                        <div className="flex flex-col items-center gap-4">
                            <div className="w-24 h-24 relative flex items-center justify-center overflow-hidden rounded-full border-2 border-emerald-600 shadow-xl bg-white dark:bg-zinc-900 p-1 mb-2">
                                <OptimizedMenuImage src={config.logo_url} alt="Logo" className="w-full h-full object-contain rounded-full" useOriginal={true} />
                            </div>
                        </div>
                    )}
                    <div className="space-y-1 opacity-50 hover:opacity-100 transition-opacity">
                        ...
                        <a href="/" target="_blank" className="text-emerald-600 font-black text-[9px] block tracking-wider">
                    ...
                    >
                        ...
                        >
                            <div className="p-6 border-b border-zinc-100 dark:border-white/5 flex items-center justify-between">
                                <button onClick={() => { setSelectedItem(null); setItemNotes(''); }} className="w-10 h-10 bg-zinc-100 dark:bg-zinc-800 rounded-full flex items-center justify-center font-black transition-all active:scale-90 hover:bg-emerald-600 hover:text-white">✕</button>
                                <div className="text-right">
                                    ...
                                    <p className="text-[10px] text-emerald-600 font-black mt-1 uppercase tracking-widest">{selectedItem.catName}</p>
                                </div>
                            </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        {(selectedItem.item.prices?.length > 0 ? selectedItem.item.prices : [0]).map((p, idx) => (
                                            <button
                                                ...
                                                className={`p-4 rounded-3xl border-2 transition-all flex flex-col items-center gap-1 ${tempSizeIdx === idx ? "border-emerald-600 bg-emerald-50 dark:bg-emerald-900/10" : "border-zinc-100 dark:border-white/5 bg-zinc-50 dark:bg-white/5"}`}
                                            >
                                                <span className={`text-[9px] font-black uppercase ${tempSizeIdx === idx ? "text-emerald-600" : "text-zinc-400"}`}>
                            ...
                            <div className="p-6 bg-zinc-50 dark:bg-white/5 border-t border-zinc-100 dark:border-white/5">
                                {config.orders_enabled !== false && (
                                    <button
                                        onClick={addToCart}
                                        className="w-full bg-emerald-600 text-white font-black py-4 rounded-3xl shadow-lg flex items-center justify-center gap-3 active:scale-95 transition-all text-base"
                                    >
            ...
            <AnimatePresence>
                {showCart && (
                    ...
                    >
                        ...
                        >
                            <div className="p-6 border-b border-zinc-100 dark:border-white/5 flex items-center justify-between">
                                <button onClick={() => setShowCart(false)} className="w-10 h-10 bg-zinc-100 dark:bg-zinc-800 rounded-full flex items-center justify-center font-black transition-all active:scale-90 hover:bg-emerald-600 hover:text-white">✕</button>
                                <div className="text-right">
                                        ...
                                        {cart.map((c) => (
                                            <div key={c.id} className="bg-zinc-50 dark:bg-white/5 p-4 rounded-3xl border border-zinc-100 dark:border-white/5 flex items-center justify-between">
                                                <div className="flex-1 text-right">
                                                    ...
                                                    <p className="text-xs font-black mt-1 text-emerald-600">{c.price * c.quantity} {config.currency || (isAr ? "ج" : "EGP")}</p>
                                                </div>
                                                <div className="flex items-center gap-2.5 bg-white dark:bg-zinc-800 p-1 rounded-xl shadow-sm border border-zinc-100 dark:border-white/10">
                                                    <button onClick={() => updateCartQty(c.id, 1)} className="w-7 h-7 flex items-center justify-center bg-emerald-600 text-white rounded-lg font-black text-sm">+</button>
                            ...
                            <div className="p-6 border-t border-zinc-100 dark:border-white/5 bg-zinc-50 dark:bg-white/5">
                                <div className="flex items-center justify-between mb-4 px-2">
                                    <div className="flex flex-col items-start">
                                        <span className="text-2xl font-black tabular-nums text-emerald-600">{cartTotal} {config.currency || (isAr ? "ج" : "EGP")}</span>
                                ...
                                <button onClick={() => { setShowCart(false); setShowCheckout(true); }}
                                    className="w-full bg-emerald-600 text-white font-black py-4 rounded-3xl shadow-lg active:scale-95 transition-all text-base flex items-center justify-center gap-3">
                ...
                {showCategoriesModal && (
                    ...
                    >
                        <div className="w-[85vw] max-w-[310px] bg-white dark:bg-[#09090b] rounded-[2rem] shadow-2xl overflow-hidden relative max-h-[85vh] flex flex-col mx-auto" onClick={(e) => e.stopPropagation()}>
                            <button onClick={() => setShowCategoriesModal(false)} className="absolute top-6 left-6 w-10 h-10 bg-zinc-100 dark:bg-zinc-800 rounded-full flex items-center justify-center font-black transition-all active:scale-90 hover:bg-emerald-600 hover:text-white z-10">✕</button>
                            ...
                            <div className="p-4 grid grid-cols-2 gap-3 max-h-[85vh] overflow-y-auto" style={{ scrollbarWidth: "none" }}>
                                {categories.map((cat) => (
                                    <button
                                        key={cat.id}
                                        onClick={() => handleNavClick(cat.id)}
                                        className="flex flex-row-reverse items-center justify-between bg-zinc-50 dark:bg-[#1c1c1e] hover:bg-emerald-600 hover:text-white p-4 rounded-xl border border-zinc-100 dark:border-white/5 active:scale-95 transition-all group"
                                    >
            ...
            <nav className="fixed bottom-0 left-0 right-0 z-[60] px-4 pb-8 pt-2">
                <div className="w-[85vw] max-w-[310px] mx-auto bg-white/80 dark:bg-zinc-900/80 backdrop-blur-2xl border border-zinc-200 dark:border-white/10 rounded-[2.5rem] p-1.5 flex items-center justify-around shadow-xl relative">
                    ...
                    <button
                        ...
                        className="bg-emerald-600 w-14 h-14 rounded-full flex items-center justify-center text-white shadow-lg -mt-10 border-4 border-white dark:border-[#050505] active:scale-90 z-[63] transition-all"
                    >
                        <span className="text-xl">🔝</span>
                    </button>
                        ...
                        <motion.div initial={{ scale: 0.95, opacity: 0, y: 50 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 50 }}
                            ...
                            <div className="p-4 overflow-y-auto flex flex-col gap-3">
                                {config.payment_methods?.map((pm) => (
                                    <div key={pm.id} className="bg-zinc-50 dark:bg-white/5 border border-zinc-200 dark:border-white/10 rounded-[1.5rem] p-4 text-right flex flex-col transition-all hover:border-emerald-600/50">
                                        ...
                                        {pm.number && (
                                            <div className="flex items-center justify-between bg-white dark:bg-black/20 px-4 py-3 rounded-xl mb-3 border border-zinc-100 dark:border-white/5 group">
                                                <button
                                                    ...
                                                    className="text-emerald-600 text-xs font-black uppercase bg-emerald-600/10 px-3 py-1.5 rounded-lg active:scale-95"
                                                >
                                                    {isAr ? "نسخ" : "Copy"}
                                                </button>
                                        ...
                                        <button
                                            ...
                                            className="w-full bg-emerald-600 text-white font-black py-3 rounded-xl active:scale-95 transition-transform shadow-md flex items-center justify-center gap-2"
                                        >
