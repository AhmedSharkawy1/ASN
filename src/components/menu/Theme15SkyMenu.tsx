...
export default function Theme15SkyMenu({ config, categories, restaurantId }: { config: RestaurantConfig, categories: CategoryWithItemsType[], restaurantId: string }) {
    ...
    // --- Cart Actions ---
    const openModal = (item: Item, cName: string, cImage: string) => {
        if (config.orders_enabled === false) return;
        setSelectedItem({ item, cName, cImage });
        setSizeIdx(0);
        setQty(1);
        setSelectedExtras([]);
        setNotes("");
        ...
        <div className="theme15-container" dir={isRTL ? 'rtl' : 'ltr'}>
                                ...
                                return (
                                    <div className="menu-item" key={item.id}>
                                        ...
                                        <div className="item-image" onClick={() => openModal(item, cName, cImage || '')}>
                                            <OptimizedMenuImage thumbnailSrc={item.thumbnail_url} originalSrc={item.image_url || cImage || '/placeholder.jpg'} alt="Item" className="w-full h-full object-cover cursor-pointer" />
                                        </div>
            ...
            {showCategorySheet && (
                <div className="modal-overlay open !items-center" onClick={() => setShowCategorySheet(false)}>
                    <div className="modal-content-sheet !rounded-xl !transform-none !transition-none max-h-[85vh] w-[90%] mx-auto" onClick={e => e.stopPropagation()}>
                ...
                <div className="modal-overlay open" onClick={closeModal}>
                    <div className="modal-content-sheet w-[85vw] max-w-[310px] w-full max-h-[85vh] flex flex-col mx-auto" onClick={e => e.stopPropagation()}>
                        <div className="relative h-32 w-full bg-gray-100">
                            <OptimizedMenuImage thumbnailSrc={null} originalSrc={selectedItem.item.image_url || selectedItem.cImage || '/placeholder.jpg'} alt="Item" useOriginal className="w-full h-full [&_img]:object-contain" />
                        ...
                        {config.orders_enabled !== false && (
                            <div className="absolute bottom-0 left-0 right-0 p-4 bg-white border-t border-gray-100 flex items-center gap-4 z-10 shadow-[0_-10px_15px_rgba(0,0,0,0.05)] rounded-b-lg">
                                <div className="flex items-center bg-gray-100 rounded-xl max-w-[120px] w-full p-1 border">
                                    <button className="w-9 h-9 flex items-center justify-center text-gray-600 font-bold text-lg" onClick={() => setQty(Math.max(1, qty - 1))}>-</button>
                                    <span className="flex-1 text-center font-bold">{qty}</span>
                                    <button className="w-9 h-9 flex items-center justify-center text-gray-600 font-bold text-lg" onClick={() => setQty(qty + 1)}>+</button>
                                </div>
                                <button className="flex-1 h-11 rounded-xl text-white font-bold flex items-center justify-center gap-2 transition-transform active:scale-95 shadow-md"
            ...
            {isCartOpen && (
                <div className="modal-overlay open" onClick={() => setIsCartOpen(false)}>
                    <div className="bg-white w-[85vw] max-w-[310px] w-full h-[90vh] rounded-3xl shadow-2xl flex flex-col transform transition-transform max-h-[85vh] flex flex-col mx-auto" onClick={e => e.stopPropagation()}>
                        <div className="p-4 border-b flex justify-between items-center bg-gray-50 rounded-3xl">
                        ...
                        {cart.length > 0 && (
                            <div className="p-4 shadow-[0_-5px_15px_rgba(0,0,0,0.05)] border-t z-20 bg-white rounded-b-2xl">
