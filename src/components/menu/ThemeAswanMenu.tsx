...
export default function ThemeAswanMenu({ config, categories, restaurantId }: ThemeAswanMenuProps) {
    ...
    const getItemDesc = (item: MenuItem) => item.description_en || item.desc_en || item.details_en || item.description || item.desc || item.description_ar || item.desc_ar || '';
                                                    ...
                                                    <div className="mt-3 flex items-center justify-between">
                                                        <div className="font-extrabold text-sm md:text-base whitespace-nowrap" style={{ color: primaryColor }}>
                                                            {item.size_labels && item.size_labels.length > 1 ? (
                                                                <span className="text-xs">From {item.prices?.[0] || 0} {cur}</span>
                                                            ) : (
