        ...
        <div className="min-h-screen pb-20" dir="rtl"
            ...
            <div className="relative flex flex-col items-center -mt-16 z-10 px-4">
                <div className="w-32 h-32 rounded-full border-4 overflow-hidden bg-white shadow-xl"
                    style={{ borderColor: isDarkMode ? T7_BG : '#059669' }}>
            ...
            <div className="sticky top-0 z-[100] mt-6" style={{ backgroundColor: isDarkMode ? T7_BG : '#f8fafc' }}>
                ...
                <div ref={catNavRef}
                    ...
                    style={{ scrollbarWidth: 'none', borderColor: isDarkMode ? T7_BORDER : '#059669' }}>
                    {categories.map((cat: any) => {
                        ...
                        return (
                            <button key={cat.id} data-cat-id={cat.id}
                                ...
                                style={{
                                    color: isActive ? T7_GOLD : (isDarkMode ? '#34d399' : '#64748b'),
                        ...
                        return (
                            <button key={cat.id} data-id={cat.id}
                                ...
                                <span className="text-xs font-bold text-center leading-tight"
                                    style={{ color: isActive ? T7_GOLD : (isDarkMode ? '#34d399' : '#64748b') }}>
                                    {isAr ? cat.name_ar : (cat.name_en || cat.name_ar)}
                                </span>
                            </button>
                        );
                            ...
                            {section.items.filter((i: any) => i.is_available !== false).map((item: any) => (
                                <div key={item.id}
                                    ...
                                    style={{
                                        ...
                                        border: `1px solid ${isDarkMode ? T7_BORDER : '#059669'}`,
                                    }}>
                                    ...
                                    <div className="flex-1 flex flex-col text-right min-h-[100px] justify-between"
                                        ...
                                        <div>
                                            ...
                                            {item.desc_ar && (
                                                <p className="text-sm leading-relaxed mb-2" style={{ color: isDarkMode ? '#34d399' : '#64748b' }}>
                                                ...
                                                return (
                                                    <div key={pIdx}
                                                        ...
                                                        style={{
                                                            ...
                                                            border: `1px solid ${isDarkMode ? 'rgba(255,255,255,0.15)' : '#059669'}`,
                                                        }}>
                    ...
                    <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }}
                        ...
                        <div className="flex-1 flex justify-end px-4 text-right">
                            <div className="flex flex-col">
                                ...
                                <span className="text-[10px] uppercase tracking-wider" style={{ color: isDarkMode ? '#34d399' : '#64748b' }}>{isAr ? 'عرض السلة' : 'View Cart'}</span>
                            </div>
                        </div>
                    </motion.div>
                )}
                        ...
                        <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }}
                            ...
                            <div className="p-5 flex-1 flex flex-col gap-5 text-right" dir="rtl">
                                <div>
                                    ...
                                    <p className="text-sm leading-relaxed" style={{ color: isDarkMode ? '#34d399' : '#64748b' }}>
                                        ...
                                        {selectedItem.item.prices.map((price: number, idx: number) => {
                                            ...
                                            return (
                                                <label key={idx}
                                                    ...
                                                    style={{
                                                        borderColor: sel ? T7_GOLD : (isDarkMode ? T7_BORDER : '#059669'),
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <div className="w-5 h-5 rounded-full border-2 flex items-center justify-center"
                                                            style={{ borderColor: sel ? T7_GOLD : (isDarkMode ? '#475569' : '#34d399') }}>
                            ...
                            {config.orders_enabled !== false && (
                                <div className="p-4 flex items-center gap-4 shrink-0 border-t"
                                    style={{ borderColor: isDarkMode ? T7_BORDER : '#059669', background: isDarkMode ? T7_BG : '#fff' }}>
                ...
                {isCartOpen && (
                    <div className="fixed inset-0 z-[400] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setIsCartOpen(false)}>
                        <motion.div initial={{ opacity: 0, scale: .95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: .95 }}
                            ...
                            onClick={e => e.stopPropagation()}>
                            {/* header */}
                            <div className="p-4 flex items-center justify-between shrink-0 border-b" style={{ borderColor: isDarkMode ? T7_BORDER : '#059669' }}>
                                <button onClick={() => setIsCartOpen(false)} className="p-2 rounded-full" style={{ color: isDarkMode ? '#34d399' : '#64748b' }}>
                                    <X className="w-5 h-5" />
                                </button>
                            ...
                            <div className="p-4 overflow-y-auto flex-1 flex flex-col gap-4 text-right" dir="rtl">
                                {cart.length === 0 ? (
                                    <div className="flex flex-col items-center justify-center py-12" style={{ color: isDarkMode ? '#475569' : '#34d399' }}>
                                        <ShoppingCart className="w-16 h-16 mb-4 opacity-50" />
                                        <p>{isAr ? 'السلة فارغة' : 'Cart is empty'}</p>
                                    </div>
                                ) : cart.map(ci => (
                                    <div key={`${ci.id}-${ci.notes}`} className="flex items-start gap-3 p-3 rounded-3xl"
                                        style={{ background: isDarkMode ? 'rgba(255,255,255,0.04)' : '#f8fafc', border: `1px solid ${isDarkMode ? T7_BORDER : '#059669'}` }}>
                                        <div className="w-14 h-14 rounded-xl shrink-0 overflow-hidden" style={{ background: isDarkMode ? '#1e293b' : '#059669' }}>
                                        ...
                                        <div className="flex-1 flex flex-col gap-1">
                                            ...
                                            {ci.notes && <p className="text-[10px] italic" style={{ color: isDarkMode ? '#64748b' : '#34d399' }}>{ci.notes}</p>}
                                            <div className="flex justify-between items-center mt-2">
                                                <div className="flex items-center justify-between rounded-lg p-1 w-24"
                                                    style={{ background: isDarkMode ? 'rgba(255,255,255,0.08)' : '#f1f5f9', border: `1px solid ${isDarkMode ? T7_BORDER : '#059669'}` }}>
                            ...
                            {config.orders_enabled !== false && cart.length > 0 && (
                                <div className="p-4 shrink-0 border-t" style={{ borderColor: isDarkMode ? T7_BORDER : '#059669' }}>
                                    <div className="flex justify-between items-center mb-4">
                                        <span className="font-bold text-lg" style={{ color: T7_GOLD }}>{cartTotal.toFixed(2)} {cur}</span>
                                        <span className="text-sm" style={{ color: isDarkMode ? '#34d399' : '#64748b' }}>{isAr ? 'الاجمالي' : 'Total'}</span>
                                    </div>
