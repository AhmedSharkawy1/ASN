 ...
 */
export default function AtyabOrientalCyanMenu({ config, categories, language, restaurantId }: Props) {
        ...
        <div className="min-h-screen bg-zinc-50 dark:bg-[#050505] text-zinc-900 dark:text-zinc-200 antialiased selection:bg-yellow-500/30" style={{ fontFamily: "'Cairo', sans-serif" }}>
                    ...
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-3 group">
                            ...
                            <div className="flex flex-col">
                                ...
                                {config.phone && (
                                    <span className="text-[#0891b2] text-[11px] font-black uppercase tracking-[0.1em] mt-1">
                                ...
                                {showCallMenu && (
                                    <>
                                        ...
                                        >
                                            ...
                                            {config.phone && (
                                                <a href={`tel:${config.phone}`} className="flex items-center justify-between px-5 py-3.5 hover:bg-[#0891b2]/5 border-b border-zinc-100 dark:border-white/5 transition-all">
            ...
            <nav className="sticky top-0 z-40 bg-white/95 dark:bg-[#050505]/95 backdrop-blur-xl border-b border-zinc-200 dark:border-white/10 shadow-sm">
                <div className="max-w-2xl mx-auto relative flex items-center">
                    <button onClick={() => scrollNav("right")} className="absolute right-0 z-10 w-10 h-full bg-gradient-to-l from-white dark:from-[#050505] to-transparent flex items-center justify-center text-zinc-400 active:text-[#0891b2] transition-all">
                        ...
                        {categories.map((cat) => (
                            <button
                                ...
                                className={`whitespace-nowrap px-4 py-2 rounded-2xl text-[12px] font-black border transition-all duration-300 ${activeCategory === cat.id
                                    ? "bg-[#0891b2] text-black border-[#0891b2] scale-105 shadow-lg shadow-[#0891b2]/20"
                                    : "bg-white dark:bg-zinc-900 border-zinc-200 dark:border-white/10 text-zinc-500 hover:border-[#0891b2]/30"
                                    }`}
                            >
                    ...
                    <button onClick={() => scrollNav("left")} className="absolute left-0 z-10 w-10 h-full bg-gradient-to-r from-white dark:from-[#050505] to-transparent flex items-center justify-center text-zinc-400 active:text-[#0891b2] transition-all">
            ...
            <main className="max-w-2xl mx-auto px-5 py-8 pb-48">
                ...
                <div className="mb-8 rounded-[1.5rem] bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 p-6 relative overflow-hidden text-right shadow-md atyab-reveal">
                    <div className="relative z-10">
                        ...
                        <p className="text-[#0891b2] text-[10px] font-black uppercase mb-2 tracking-widest">{isAr ? "مذاقات أصيلة" : "Authentic Flavors"}</p>
                    </div>
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`py-2 group transition-all -mx-2 px-2 rounded-2xl border border-transparent cursor-pointer flex gap-2
                                                ...
                                                hover:bg-[#0891b2]/5 hover:border-[#0891b2]/10`}
                                        >
                                            {/* Name + Description */}
                                            <div className={`flex flex-col gap-0.5 flex-1 min-w-0 text-right ${hasManyPrices ? "w-full" : "pr-1"}`}>
                                                <div className="flex flex-wrap items-center justify-end gap-2">
                                                    <div className="flex items-center gap-1 order-1">
                                                        {item.is_popular && <span className="bg-[#0891b2] text-black text-[8px] font-black px-1.5 py-0.5 rounded-md atyab-popular">{isAr ? "مميز" : "Popular"}</span>}
                                                        {item.is_spicy && <span className="atyab-spicy text-xs leading-none">🌶️</span>}
                                                    </div>
                                                    <span className="text-zinc-900 dark:text-zinc-100 font-black text-lg leading-tight group-hover:text-[#0891b2] transition-colors order-2 text-right w-full">
                                                ...
                                                <div className={`${hasManyPrices ? "grid grid-cols-3 gap-1.5 w-full mt-1" : "flex gap-2 items-end shrink-0"}`}>
                                                    {item.prices.map((price, pIdx) => (
                                                        <div key={pIdx} className={`flex flex-col items-center gap-0.5 ${hasManyPrices ? "bg-zinc-100 dark:bg-[#1a1a1a] border border-zinc-200 dark:border-white/10 rounded-xl py-1.5 px-1" : ""}`}>
                                                            ...
                                                            <div className={`${hasManyPrices ? "" : "px-2 py-1 min-w-[45px] rounded-xl border bg-zinc-100 dark:bg-zinc-800/80 group-hover:bg-white dark:group-hover:bg-zinc-800 border-zinc-200 dark:border-white/5 group-hover:border-[#0891b2]/40 transition-all flex items-center justify-center gap-1 shadow-sm"}`}>
                                                                <div className="flex items-center justify-center gap-0.5">
                                                                    <span className={`${hasManyPrices ? "text-[#0891b2] text-lg" : "text-[#0891b2] text-base"} font-black leading-none tabular-nums`}>{price}</span>
                                                                    <span className={`${hasManyPrices ? "text-[#0891b2]/60 text-[9px]" : "text-zinc-500 text-[8px]"} font-black`}>{currency}</span>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    ))}
                                                ...
                                                item.prices.length > 0 && (
                                                    <div className="flex items-end shrink-0">
                                                        <div className="px-2 py-1 min-w-[45px] rounded-xl border bg-zinc-100 dark:bg-zinc-800/80 border-zinc-200 dark:border-white/5 flex items-center justify-center gap-1 shadow-sm">
                                                            <div className="flex items-center justify-center gap-0.5">
                                                                <span className="text-[#0891b2] text-base font-black leading-none tabular-nums">{item.prices[0]}</span>
                ...
                <footer className="mt-16 pb-12 flex flex-col items-center gap-10 atyab-reveal">
                        ...
                        {config.map_link && (
                            <div className="w-full border-t border-zinc-100 dark:border-white/5 pt-8">
                                <a href={config.map_link} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center group transition-transform active:scale-95">
                                    ...
                                    <h4 className="text-xl font-black text-zinc-900 dark:text-white group-hover:text-[#0891b2] transition-colors leading-none">{isAr ? "موقعنا على الخريطة" : "Find us on Map"}</h4>
                                </a>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="flex flex-col items-center gap-1.5 opacity-60 hover:opacity-100 transition-opacity">
                        <a href="/" target="_blank" className="text-[11px] font-bold text-zinc-500 dark:text-zinc-400">
                            {isAr ? "مدعوم بواسطة" : "Powered by"} <span className="font-black text-[#0891b2]">ASN Technology</span>
                        </a>
                    </div>
                </footer>
            </main>
                        ...
                        <motion.div
                            ...
                            <div className="p-6 border-b border-zinc-200 dark:border-white/10 flex items-center justify-between">
                                ...
                                <div className="text-right">
                                    <h3 className="text-xl font-black text-[#0891b2]">{isAr ? selectedItem.item.title_ar : selectedItem.item.title_en || selectedItem.item.title_ar}</h3>
                                    <p className="text-[10px] text-[#0891b2]/60 font-black uppercase tracking-widest">{selectedItem.cName}</p>
                                </div>
                            </div>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    {(selectedItem.item.prices.length > 0 ? selectedItem.item.prices : [0]).map((p, idx) => (
                                        <button key={idx} onClick={() => { setTempSizeIdx(idx); haptic(5); }}
                                            className={`p-4 rounded-3xl border-2 transition-all flex flex-col items-center gap-1 ${tempSizeIdx === idx ? "border-[#0891b2] bg-[#0891b2]/10" : "border-transparent bg-zinc-100 dark:bg-white/5"}`}>
                                            <span className={`text-[10px] font-black uppercase ${tempSizeIdx === idx ? "text-[#0891b2]" : "opacity-60"}`}>{selectedItem.item.size_labels?.[idx] || "عادي"}</span>
                            ...
                            <div className="p-6 bg-zinc-50 dark:bg-white/5 border-t border-zinc-200 dark:border-white/10">
                                <button onClick={addToCart}
                                    className="w-full bg-[#0891b2] text-black font-black py-4 rounded-2xl shadow-lg flex items-center justify-center gap-3 active:scale-95 transition-all text-lg">
                        ...
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
                                    ...
                                    {cart.map((c) => (
                                        <div key={c.id} className="bg-zinc-50 dark:bg-white/5 p-4 rounded-2xl flex items-center justify-between">
                                            <div className="flex-1 text-right">
                                                ...
                                                <p className="text-xs font-black mt-1 text-[#0891b2]">{c.price * c.quantity} ج.م</p>
                                            </div>
                                            <div className="flex items-center gap-2.5 bg-zinc-100 dark:bg-black/40 p-1 rounded-xl shadow-sm border border-zinc-200 dark:border-white/5">
                                                <button onClick={() => updateCartQty(c.id, 1)} className="w-7 h-7 flex items-center justify-center bg-[#0891b2] text-black rounded-lg font-black text-sm">+</button>
                            ...
                            <div className="p-6 border-t border-zinc-200 dark:border-white/10 bg-zinc-50 dark:bg-white/5">
                                <div className="flex items-center justify-between mb-4 px-2">
                                    <span className="text-2xl font-black tabular-nums text-[#0891b2]">{cartTotal} {isAr ? "ج.م" : "EGP"}</span>
                                ...
                                <button onClick={() => { setShowCart(false); setShowCheckout(true); }}
                                    className="w-full bg-[#0891b2] text-black font-black py-4 rounded-2xl shadow-lg active:scale-95 transition-all text-base flex items-center justify-center gap-3">
            ...
            <AnimatePresence>
                {cart.length > 0 && !showCart && !selectedItem && (
                    <motion.button
                        ...
                        className="fixed bottom-28 left-1/2 -translate-x-1/2 z-[58] bg-[#0891b2] text-black shadow-2xl shadow-[#0891b2]/30 px-6 py-4 rounded-full flex items-center gap-3 transition-transform hover:scale-105 active:scale-95 font-black">
                        <ShoppingCart className="w-5 h-5" />
                        <span>{isAr ? "سلة الطلبات" : "Cart"} ({cart.length})</span>
                        <span className="bg-black text-[#0891b2] px-3 py-1 rounded-full text-sm">{cartTotal} {currency}</span>
                    </motion.button>
                )}
            </AnimatePresence>
            ...
            <nav className="fixed bottom-0 left-0 right-0 z-[60] px-4 pb-8 pt-2 md:hidden">
                <div className="max-w-xl mx-auto bg-white/80 dark:bg-[#0f0f0f]/70 backdrop-blur-xl border border-zinc-200 dark:border-white/10 rounded-[2.5rem] p-1 flex items-center justify-around shadow-xl relative">
                    ...
                    <button onClick={() => { haptic(); setShowCallMenu(!showCallMenu); }} className={`flex-1 flex flex-col items-center py-2 active:scale-90 transition-all ${showCallMenu ? "text-[#0891b2]" : "text-zinc-500"}`}>
                        <span className="text-xl atyab-emoji">📞</span>
                        <span className="text-[8px] font-black text-zinc-400 mt-0.5">{isAr ? "اتصال" : "Call"}</span>
                    </button>

                    <button onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} className="bg-[#0891b2] w-14 h-14 rounded-full flex items-center justify-center text-black shadow-lg -mt-10 border-4 border-white dark:border-[#050505] active:scale-90 z-[63] transition-all">
                            ...
                            <div className="p-4 grid grid-cols-2 gap-3 max-h-[60vh] overflow-y-auto">
                                {categories.map((cat) => (
                                    <button key={cat.id} onClick={() => handleNavClick(cat.id)}
                                        ...
                                        <span className="text-[13px] font-bold text-white group-hover:text-[#0891b2] transition-colors">{isAr ? cat.name_ar : cat.name_en || cat.name_ar}</span>
                        ...
                        <motion.div initial={{ scale: 0.95, opacity: 0, y: 50 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 50 }}
                            ...
                            <div className="p-4 overflow-y-auto flex flex-col gap-3">
                                {config.payment_methods?.map((pm) => (
                                    <div key={pm.id} className="bg-zinc-50 dark:bg-white/5 border border-zinc-200 dark:border-white/10 rounded-[1.5rem] p-4 text-right flex flex-col transition-all hover:border-[#0891b2]/50">
                                        ...
                                        {pm.number && (
                                            <div className="flex items-center justify-between bg-white dark:bg-black/20 px-4 py-3 rounded-xl mb-3 border border-zinc-100 dark:border-white/5 group">
                                                <button
                                                    ...
                                                    className="text-[#0891b2] text-xs font-black uppercase bg-[#0891b2]/10 px-3 py-1.5 rounded-lg active:scale-95"
                                                >
                                                    {isAr ? "نسخ" : "Copy"}
                                                </button>
                                        ...
                                        <button
                                            ...
                                            className="w-full bg-[#0891b2] text-black font-black py-3 rounded-xl active:scale-95 transition-transform shadow-md flex items-center justify-center gap-2"
                                        >
