...
import { ArrowLeft, ArrowRight, Phone, Clock, Instagram, Facebook, Youtube, Sun, Moon, Sparkles, ChefHat, Award, Globe } from 'lucide-react';
...
interface UsaDualLandingPageProps {
    config: any;
    onContinue: () => void;
    lang?: 'en' | 'ar';
    onLangToggle?: () => void;
...
export default function UsaDualLandingPage({ config, onContinue, lang = 'en', onLangToggle }: UsaDualLandingPageProps) {
    const { theme, resolvedTheme, setTheme } = useTheme();
    const [mounted, setMounted] = React.useState(false);
    const [forcedMode, setForcedMode] = React.useState<'light' | 'dark' | null>(null);
    const [showPhoneModal, setShowPhoneModal] = React.useState(false);
    const [currentLang, setCurrentLang] = React.useState<'en' | 'ar'>(lang);
    ...
    const { primaryColor, bgBody, textMain, bgCard, borderColor, hasBgImage, activeBgImage } = getUsaDualColors(config, isDark);

    const displayNumbers = (config.phone_numbers && config.phone_numbers.length > 0) 
        ? config.phone_numbers 
        : (config.phone ? [{ label: isAr ? 'رقم التواصل' : 'Phone', number: config.phone }] : []);
    ...
    const hasContent = aboutText || historyText || (config.vicino_images && config.vicino_images.length > 0);
    ...
    const socialLinks = [
        { icon: FaWhatsapp, url: config.whatsapp_number ? `https://wa.me/${config.whatsapp_number.replace(/\+/g, '')}` : null, color: '#25D366', name: isAr ? 'واتساب' : 'WhatsApp' },
    ...
    return (
        ...
        >
            ...
            {/* Header & Prominent Centered Brand Logo */}
            <div className="w-full max-w-4xl mx-auto px-6 pt-6 pb-4 text-center flex flex-col items-center justify-center">
                ...
                <div className="w-full flex justify-between items-center mb-6">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            onClick={toggleLang}
                            className="p-2.5 px-3 rounded-2xl bg-slate-800/80 border border-slate-700/60 text-slate-200 hover:text-white transition-colors shadow-sm text-xs font-bold flex items-center gap-1.5"
                        >
                ...
                <span className="text-xs sm:text-sm font-extrabold text-rose-500 tracking-widest uppercase">
                    {isAr ? 'ثيم USA الثنائي 🇺🇸 - المنيو الرسمي' : 'USA DUAL THEME 🇺🇸 - OFFICIAL MENU'}
                </span>
            </div>
            ...
            <div className="w-full px-6 max-w-4xl mx-auto mb-10 text-center">
                <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    className="w-full py-4 px-8 rounded-2xl font-black text-base md:text-lg tracking-wide shadow-xl flex items-center justify-center gap-3 text-white transition-all"
                    style={{ backgroundColor: primaryColor }}
                >
                    <span>{isAr ? 'تصفح المنيو واطلب أونلاين' : 'Explore Menu & Order Online'}</span>
                    {isAr ? <ArrowLeft className="w-5 h-5 animate-pulse" /> : <ArrowRight className="w-5 h-5 animate-pulse" />}
                </motion.button>
            </div>
                    ...
                    {aboutText && (
                        ...
                        >
                            <div className="flex items-center gap-2 text-rose-500 font-bold text-xs uppercase tracking-widest">
                                ...
                                <span>{isAr ? 'عن المكان والخبرة' : 'Culinary Experience'}</span>
                            </div>
                            <h2 className="text-xl md:text-2xl font-extrabold tracking-tight">
                                {isAr ? `عن ${config.name}` : `About ${config.name}`}
                            </h2>
                    ...
                    {historyText && (
                        ...
                        >
                            <div className="flex items-center gap-2 text-rose-500 font-bold text-xs uppercase tracking-widest">
                                ...
                                <span>{isAr ? 'القصة والعراقة' : 'Our Story & Legacy'}</span>
                            </div>
                            <h2 className="text-xl md:text-2xl font-extrabold tracking-tight">
                    ...
                    {config.vicino_images && config.vicino_images.length > 0 && (
                        ...
                        >
                            <div className="flex items-center gap-2 text-rose-500 font-bold text-xs uppercase tracking-widest px-1">
                                ...
                                <span>{isAr ? 'معرض الصور والأجواء' : 'Photo Gallery'}</span>
                            </div>
                            <Swiper
            ...
            <div className="w-full px-6 max-w-4xl mx-auto mb-12 grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {(config.work_hours_en || config.work_hours_ar || config.opening_hours) && (
                    ...
                    >
                        ...
                        <div className="space-y-1">
                            <h3 className="font-bold text-base">{isAr ? 'ساعات العمل' : 'Opening Hours'}</h3>
                            <p className="text-xs opacity-75 leading-relaxed">
                                {isAr ? (config.work_hours_ar || config.work_hours_en || config.opening_hours) : (config.work_hours_en || config.work_hours_ar || config.opening_hours)}
                            </p>
                        </div>
                    </div>
                )}
                ...
                {displayNumbers.length > 0 && (
                    ...
                    >
                        ...
                        <div className="space-y-1">
                            <h3 className="font-bold text-base">{isAr ? 'التواصل الهاتفي' : 'Contact Us'}</h3>
                            <p className="text-xs opacity-75">{isAr ? 'اضغط لعرض أرقام الهاتف والاتصال الفوري' : 'Click to view available phone numbers'}</p>
                        </div>
                    </div>
                )}
            ...
            {socialLinks.length > 0 && (
                <div className="w-full px-6 max-w-4xl mx-auto mb-12 text-center space-y-4">
                    <span className="text-xs font-semibold uppercase tracking-wider opacity-75">{isAr ? 'تابعنا على وسائل التواصل' : 'Follow Us'}</span>
            ...
            <div className="w-full py-8 text-center border-t text-xs opacity-60" style={{ borderColor }}>
                <p>&copy; {new Date().getFullYear()} {config.name}. {isAr ? 'جميع الحقوق محفوظة.' : 'All rights reserved.'}</p>
            </div>
                    ...
                    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
                        ...
                        >
                            <div className="flex justify-between items-center border-b pb-3" style={{ borderColor }}>
                                <h3 className="font-bold text-base">{isAr ? 'اتصل بنا مباشرة' : 'Call Us'}</h3>
                            ...
                            <div className="space-y-2">
                                {displayNumbers.map((numObj: any, idx: number) => (
                                    ...
                                    >
                                        <span>{numObj.label || (isAr ? 'الهاتف' : 'Phone')}</span>
                                        <span className="text-rose-500 font-bold ltr">{numObj.number}</span>
                                    </a>
                                ))}
                            </div>
                        </motion.div>
