                                ...
                                {cat.items?.map((item: MenuItem, idx: number) => (
                                    <div key={item.id || idx} onClick={() => openModal(item, catName(cat), cat.image_url)}
                                        ...
                                        <div className="w-[110px] sm:w-[130px] h-24 shrink-0 relative overflow-hidden bg-slate-100 dark:bg-slate-800">
                                            <OptimizedMenuImage src={item.image_url || cat.image_url || "https://images.unsplash.com/photo-1546069901-ba9599a7e63c?q=80&w=400"} alt={itemName(item)} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
