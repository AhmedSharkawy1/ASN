...
export default function Theme10CyanMenu({ config, categories, restaurantId }: Theme10MenuProps) {
    ...
    const bgBody = isDark ? '#0891b2' : '#f8fafc'; // slate-900 / slate-50
    const bgCard = isDark ? '#1e293b' : '#ffffff'; // slate-800 / white
    const textMain = isDark ? '#f1f5f9' : '#0891b2'; // slate-100 / slate-900
    ...
    return (
        <div className={`min-h-screen font-tajawal pb-24`} style={{ backgroundColor: bgBody, color: textMain }} dir={isAr ? 'rtl' : 'ltr'}>

            {/* Header Marquee */}
            {config.marquee_enabled && (
                <div className="bg-red-600 font-tajawal text-sm text-white">
                    <SharedMarquee
            ...
            <main className="max-w-[1200px] mx-auto px-4 w-full">
                ...
                <div className="pt-6 relative z-10">
                    <div className="rounded-3xl md:rounded-3xl overflow-hidden shadow-lg border relative aspect-[21/9] sm:aspect-[21/7] max-h-[350px] w-full" style={{ borderColor }}>
                        ...
                        <div key={cat.id} id={cat.id.toString()} className="scroll-mt-32">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                                {cat.items?.map((item: MenuItem, idx: number) => (
                                    <div key={item.id || idx} onClick={() => openModal(item, catName(cat), cat.image_url)}
                                        className="rounded-3xl overflow-hidden border transition-all hover:shadow-md cursor-pointer group flex flex-row"
                                        style={{ backgroundColor: bgCard, borderColor }}>
                                        {/* Image Box */}
                                        <div className="w-[110px] sm:w-[130px] h-24 shrink-0 relative overflow-hidden bg-slate-100 dark:bg-slate-800">
                                            <OptimizedMenuImage thumbnailSrc={item.thumbnail_url} originalSrc={item.image_url || cat.image_url || "https://images.unsplash.com/photo-1546069901-ba9599a7e63c?q=80&w=400"} alt={itemName(item)} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                                        ...
                                        <div className="flex-1 p-3 flex flex-col min-w-0">
                                            ...
                                            <div className=" pt-2 border-t flex flex-wrap items-end gap-x-2 gap-y-1" style={{ borderColor }}>
                                                {item.prices.map((price: number, pIdx: number) => (
                                                    <div key={pIdx} className="flex flex-col items-start bg-black/5 dark:bg-white/5 rounded-md px-2 py-1 flex-1 min-w-[60px]">
                                                        {item.size_labels?.[pIdx] && (
                                                            <span className="text-[10px] opacity-70 mb-0.5 whitespace-nowrap" style={{ color: textMuted }}>
                                                                {item.size_labels[pIdx]}
                                                            </span>
                                                        )}
                                                        <div className="font-w600 text-[13px] tracking-tighter whitespace-nowrap" style={{ color: primaryColor }}>
