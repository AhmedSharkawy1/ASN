        ...
        <div className={`min-h-screen font-sans transition-colors duration-300`} style={{ backgroundColor: bgBody, color: textMain, '--primary-color': getPrimaryColor() } as React.CSSProperties} dir={isAr ? 'rtl' : 'ltr'}>
            ...
            <div className="w-full bg-white dark:bg-[#1a1a2e] py-4 flex flex-col items-center justify-center gap-3">
                <div className="w-20 h-20 md:w-24 md:h-24 relative rounded-full overflow-hidden border-4 border-[#6c63ff] p-1 bg-white shadow-md">
                    <OptimizedMenuImage src={getLogo()} alt={getRestaurantName()} className="w-full h-full object-cover rounded-full" />
                </div>
                                ...
                                {coverImages.map((src, i) => (
                                    <SwiperSlide key={i} className="w-full h-full">
                                        <div className="relative w-full h-full">
                                            <OptimizedMenuImage src={src} alt={`Banner ${i + 1} `} className="w-full h-full object-cover" />
                                ...
                                {cart.map((c) => (
                                    <div key={c.id} className="bg-white dark:bg-[#1a1a2e] border border-gray-100 dark:border-white/10 p-4 rounded-xl flex gap-4 shadow-sm hover:shadow-md transition-shadow">
                                        <div className="w-20 h-20 rounded-lg overflow-hidden flex-shrink-0 relative border border-gray-100 dark:border-white/10">
                                            <OptimizedMenuImage src={c.item.image_url || c.catImg || 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?q=80&w=200'} alt={itemName(c.item)} className="w-full h-full object-cover" />
                                        </div>
