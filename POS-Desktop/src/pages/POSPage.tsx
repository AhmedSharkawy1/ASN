...
import type { Category, MenuItem, Customer, PosUser } from '../lib/db';
...
import {
    ...
    LayoutGrid, Receipt, CheckCircle2, Volume2, VolumeX, Package, Truck
} from 'lucide-react';
...
type Props = { user: AppUser; restaurantName: string; restaurantPhone?: string };

export default function POSPage({ user, restaurantName, restaurantPhone }: Props) {
    ...
    const loadData = useCallback(async () => {
        ...
        const todayOrders = allOrders.filter(o => o.created_at.startsWith(todayStr) && o.status !== 'cancelled' && o.status !== 'held');
        setTodayStats({ count: todayOrders.length, revenue: todayOrders.reduce((s, o) => s + o.total, 0) });

        const held = allOrders.filter(o => o.status === 'held');
        ...
        const driverList = await db.pos_users.where('role').equals('delivery').and(u => u.is_active).toArray();
        setDrivers(driverList);

        const custs = await db.customers.toArray();
        // Also build customer list from orders
        const orderCustMap: Record<string, Customer> = {};
        allOrders.forEach(o => {
            if (!o.customer_name) return;
            const key = o.customer_phone || o.customer_name;
            if (!orderCustMap[key]) orderCustMap[key] = { name: o.customer_name, phone: o.customer_phone || '', created_at: o.created_at };
        });
        const allCusts = [...custs];
        Object.values(orderCustMap).forEach(oc => {
            if (!allCusts.find(c => c.phone === oc.phone || c.name === oc.name)) allCusts.push(oc);
        });
    ...
    const clearCart = () => { setCart([]); setDiscountValue(0); setCustomerName(''); setCustomerPhone(''); setCustomerAddress(''); setOrderNotes(''); setPaymentMethod('cash'); setSelectedDriver(''); setDeliveryFee(0); };
    ...
    const submitOrder = useCallback(async (isHold = false) => {
        ...
        try {
            ...
            await db.orders.add({
                order_number: orderNumber, items, subtotal, discount, total,
            ...
            // Auto-save customer if new, or update address if they exist
    ...
    }, [cart, subtotal, discount, total, paymentMethod, customerName, customerPhone, selectedDriver, drivers, submitting, loadData]);
    ...
    /* ═══════════════════════════ RENDER ═══════════════════════════ */
    return (
        <div className="flex flex-col h-full -m-4 p-4 relative" dir="rtl">
            {/* TOP BAR */}
            <div className="flex items-center justify-between gap-3 mb-4 flex-wrap">
                <h1 className="text-2xl font-extrabold text-white flex items-center gap-2"><CreditCard className="w-6 h-6 text-emerald-400" /> POS</h1>
                <div className="flex items-center gap-3 flex-wrap">
                    <div className="flex items-center gap-1.5 bg-[#0d1117] border border-zinc-800/50 rounded-xl px-3 py-2"><Receipt className="w-3.5 h-3.5 text-emerald-400" /><span className="text-[10px] text-zinc-500 font-bold">اليوم</span><span className="text-xs font-extrabold text-white">{todayStats.count}</span></div>
                    <div className="flex items-center gap-1.5 bg-[#0d1117] border border-zinc-800/50 rounded-xl px-3 py-2"><Banknote className="w-3.5 h-3.5 text-emerald-400" /><span className="text-[10px] text-zinc-500 font-bold">الإيرادات</span><span className="text-xs font-extrabold text-emerald-400">{formatCurrency(todayStats.revenue)}</span></div>
                    <button onClick={() => setShowHeld(!showHeld)} className={`flex items-center gap-1.5 px-3 py-2 rounded-xl border text-xs font-bold transition ${showHeld ? 'bg-amber-500/20 text-amber-400 border-amber-500/30' : 'bg-[#0d1117] text-zinc-400 border-zinc-800/50 hover:text-white'}`}><PauseCircle className="w-3.5 h-3.5" /> معلقة ({heldOrders.length})</button>
                    <button onClick={() => setSoundEnabled(!soundEnabled)} className="w-9 h-9 flex items-center justify-center bg-[#0d1117] border border-zinc-800/50 rounded-xl text-zinc-500 hover:text-white transition">{soundEnabled ? <Volume2 className="w-4 h-4" /> : <VolumeX className="w-4 h-4" />}</button>
                    <div className="flex items-center gap-1.5 bg-[#0d1117] border border-zinc-800/50 rounded-xl px-3 py-2"><Clock className="w-3.5 h-3.5 text-zinc-500" /><span className="text-xs font-bold text-zinc-300 tabular-nums">{currentTime.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}</span></div>
                </div>
            </div>
            ...
            <div className="flex flex-1 gap-4 min-h-0">
                ...
                <div className="flex-1 flex flex-col min-h-0 min-w-0">
                    <div className="relative mb-3">
                        ...
                        <input ref={searchRef} value={searchQ} onChange={e => setSearchQ(e.target.value)} placeholder="بحث عن صنف... (/)" className="w-full pr-10 pl-4 py-2.5 bg-[#0d1117] border border-zinc-800/50 rounded-xl text-sm text-white placeholder:text-zinc-600 outline-none focus:border-emerald-500/50 transition" />
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                        <button onClick={() => setActiveCategory('all')} className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 ${activeCategory === 'all' ? 'bg-emerald-500/20 text-emerald-400 border-2 border-emerald-500/40 shadow-lg shadow-emerald-500/10' : 'bg-[#0d1117] text-zinc-400 border border-zinc-800/50 hover:text-white'}`}><LayoutGrid className="w-3.5 h-3.5" /> الكل <span className="bg-zinc-700/50 text-zinc-400 px-1.5 py-0.5 rounded-md text-[10px] font-bold">{menuItems.length}</span></button>
                        {categories.map(cat => (
                            <button key={cat.id} onClick={() => setActiveCategory(cat.id!)} className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 ${activeCategory === cat.id ? 'bg-emerald-500/20 text-emerald-400 border-2 border-emerald-500/40 shadow-lg shadow-emerald-500/10' : 'bg-[#0d1117] text-zinc-400 border border-zinc-800/50 hover:text-white'}`}>
                                {cat.image_data ? <img src={cat.image_data} alt="" className="w-4 h-4 rounded object-cover" /> : cat.emoji && <span>{cat.emoji}</span>} {cat.name_ar}
                    ...
                    <div className="flex-1 overflow-y-auto grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 2xl:grid-cols-5 gap-3 content-start pb-2 auto-rows-max" style={{ scrollbarWidth: 'thin', scrollbarColor: '#333 transparent' }}>
                        {filteredItems.length === 0 ? (
                            ...
                            const imgSrc = item.image_data || getCatImage(item.category_id);
                            return (
                                <button key={item.id} onClick={() => handleItemClick(item)} className="bg-[#0d1117] border border-zinc-800/60 rounded-xl p-2 text-right transition-all group relative overflow-hidden hover:border-emerald-500/40 hover:shadow-md hover:shadow-emerald-500/5 active:scale-95 cursor-pointer flex flex-col">
                                    {imgSrc ? (
                                    ...
                                    <p className="text-xs font-bold text-zinc-200 group-hover:text-emerald-400 transition leading-tight mb-1">{item.title_ar}</p>
                                    ...
                                    {item.is_popular && <div className="absolute top-1 right-1 text-[8px] bg-amber-500/20 text-amber-400 px-1.5 py-0.5 rounded font-bold z-10 shadow-sm border border-amber-500/20">⭐ مميز</div>}
                                </button>
                            );
                        })}
                    </div>
                ...
                <div className="w-full lg:w-[400px] bg-[#0d1117] border border-zinc-800/50 rounded-xl flex flex-col min-h-0 shrink-0">
                    <div className="p-4 border-b border-zinc-800/50 flex items-center justify-between">
                        <h2 className="font-extrabold text-white flex items-center gap-2"><ShoppingCart className="w-5 h-5 text-emerald-400" /> السلة <span className="text-emerald-400 text-sm">({cartCount})</span></h2>
                            ...
                            <div key={`${c.menuItem.id}-${c.selectedSizeIdx}-${i}`} className="bg-black/20 rounded-lg p-2.5 group/item">
                                <div className="flex items-center gap-2">
                                    <div className="flex-1 min-w-0">
                                        <p className="text-xs font-bold text-zinc-200 truncate">{c.menuItem.title_ar}{c.menuItem.size_labels && c.menuItem.size_labels.length > 1 && <span className="text-zinc-500"> ({c.menuItem.size_labels[c.selectedSizeIdx]})</span>}</p>
                        ...
                        <div className="border-t border-zinc-800/50 p-4 space-y-3">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <div className="relative">
                                    ...
                                    <input ref={customerInputRef} value={customerName} onChange={e => { setCustomerName(e.target.value); setShowCustomerSuggestions(true); }}
                                    ...
                                    {showCustomerSuggestions && customerSuggestions.length > 0 && (
                                        <div className="absolute top-full mt-1 left-0 right-0 bg-[#0d1117] border border-zinc-700 rounded-lg overflow-hidden z-50 shadow-xl">
                                            {customerSuggestions.map((c, i) => (
                                                <button key={i} onClick={() => selectCustomer(c)} className="w-full text-right px-3 py-2 hover:bg-emerald-500/10 transition flex items-center justify-between z-50 relative cursor-pointer">
                                                    <span className="text-xs font-bold text-zinc-300 pointer-events-none">{c.name}</span>
                            ...
                            {selectedDriver !== '' && (
                                <div className="grid grid-cols-1 gap-2">
                                    <input value={customerAddress} onChange={e => setCustomerAddress(e.target.value)} placeholder="عنوان التوصيل (مطلوب للدليفري)" className="w-full px-2.5 py-2 bg-amber-500/10 border border-amber-500/30 rounded-lg text-xs text-white placeholder:text-amber-500/50 outline-none focus:border-amber-500 transition" />
                                    <input type="number" value={deliveryFee || ''} onChange={e => setDeliveryFee(Number(e.target.value))} placeholder="حساب الدليفري (ج.م) - مطلوب" min="0" className="w-full px-2.5 py-2 bg-black/30 border border-zinc-800 rounded-lg text-xs text-white placeholder:text-zinc-600 outline-none focus:border-zinc-600 transition" />
                                </div>
                            )}

                            <div className="flex items-center gap-2">
                                <button onClick={() => setDiscountType(discountType === 'fixed' ? 'percent' : 'fixed')} className="w-8 h-8 bg-zinc-700/50 text-zinc-400 rounded-lg flex items-center justify-center hover:text-white transition">{discountType === 'percent' ? <Percent className="w-3.5 h-3.5" /> : <DollarSign className="w-3.5 h-3.5" />}</button>
                                ...
                                {heldOrders.length === 0 ? <div className="text-center py-12 text-zinc-600"><PauseCircle className="w-10 h-10 mx-auto mb-2 opacity-20" /><p className="text-xs">لا توجد طلبات معلقة</p></div> :
                                    heldOrders.map(o => (
                                        <div key={o.id} className="bg-[#0d1117] border border-zinc-800/50 rounded-xl p-3">
                                            ...
                                            <div className="flex gap-1.5"><button onClick={() => restoreHeldOrder(o)} className="flex-1 flex items-center justify-center gap-1 py-1.5 bg-emerald-500/20 text-emerald-400 text-[10px] font-bold rounded-lg hover:bg-emerald-500/30 transition"><Play className="w-3 h-3" /> استعادة</button><button onClick={() => deleteHeldOrder(o.id!)} className="px-3 py-1.5 bg-red-500/10 text-red-400 text-[10px] font-bold rounded-lg hover:bg-red-500/20 transition"><Trash2 className="w-3 h-3" /></button></div>
                                        </div>
                                    ))}
                            </div>
                        </div>
                    ...
                    <div className="bg-[#0d1117] border border-zinc-800/50 rounded-2xl w-full max-w-xs shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="p-3 space-y-2">
                            ...
                            {sizePickerItem.prices.map((price, idx) => (
                                <button key={idx} onClick={() => { addToCart(sizePickerItem, idx); setSizePickerItem(null); }} className="w-full flex items-center justify-between p-3 bg-black/20 border border-zinc-800/50 rounded-xl hover:border-emerald-500/30 hover:bg-emerald-500/5 transition active:scale-95 text-right mt-2">
            ...
            {/* RECEIPT — with category name and customer info */}
                    ...
                    <div className="bg-white rounded-2xl w-full max-w-xs shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
                        <div ref={receiptRef} className="p-4 text-sm text-black bg-white" dir="rtl" style={{ fontFamily: "'Courier New', monospace" }}>
                            <div style={{ textAlign: 'center', marginBottom: '15px' }}>
                                <p style={{ fontWeight: 'bold', fontSize: '22px', margin: '0 0 5px 0' }}>{restaurantName || 'Restaurant'}</p>
                                {restaurantPhone && <p style={{ fontSize: '13px', margin: '0 0 5px 0', direction: 'ltr' }}>📞 {restaurantPhone}</p>}
                            ...
                            {lastOrderNotes && (
                                <>
                                    <div style={{ borderTop: '1.5px dashed #000', margin: '12px 0' }} />
                                    <div style={{ fontSize: '14px' }}>
                                        <p style={{ margin: '2px 0' }}>ملاحظات: <strong>{lastOrderNotes}</strong></p>
                                    </div>
                                </>
                            )}
                            <div style={{ borderTop: '1.5px dashed #000', margin: '12px 0' }} />
                            ...
                            {lastDeliveryFee > 0 && <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', color: '#0891b2' }}><span>🚚 حساب الدليفري ({lastDriverName})</span><span>+{formatCurrency(lastDeliveryFee)}</span></div>}
